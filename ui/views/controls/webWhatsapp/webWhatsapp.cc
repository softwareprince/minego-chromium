...
WebWhatsapp::WebContentsCreator* GetCreatorForTesting() {
  static base::NoDestructor<WebWhatsapp::WebContentsCreator> creator;
  return creator.get();
}

// Updates the parent accessible object on the NativeView. As WebWhatsapp overrides
...
WebWhatsapp::ScopedWebContentsCreatorForTesting::ScopedWebContentsCreatorForTesting(
    WebContentsCreator creator) {
  DCHECK(!*GetCreatorForTesting());
  *GetCreatorForTesting() = creator;
}

WebWhatsapp::ScopedWebContentsCreatorForTesting::
    ~ScopedWebContentsCreatorForTesting() {
  *GetCreatorForTesting() = WebWhatsapp::WebContentsCreator();
}

////////////////////////////////////////////////////////////////////////////////
// WebWhatsapp, public:

WebWhatsapp::WebWhatsapp(content::BrowserContext* browser_context) {
...
content::WebContents* WebWhatsapp::GetWebContents(base::Location creator_location) {
  if (!web_contents()) {
    if (!browser_context_)
      return nullptr;
...
void WebWhatsapp::SetWebContents(content::WebContents* replacement) {
  TRACE_EVENT0("views", "WebWhatsapp::SetWebContents");
...
content::BrowserContext* WebWhatsapp::GetBrowserContext() {
  return browser_context_;
}

void WebWhatsapp::SetBrowserContext(content::BrowserContext* browser_context) {
  browser_context_ = browser_context;
}

void WebWhatsapp::LoadInitialURL(const GURL& url) {
...
void WebWhatsapp::SetFastResize(bool fast_resize) {
  holder_->set_fast_resize(fast_resize);
}

void WebWhatsapp::EnableSizingFromWebContents(const gfx::Size& min_size,
...
void WebWhatsapp::SetCrashedOverlayView(View* crashed_overlay_view) {
...
////////////////////////////////////////////////////////////////////////////////
// WebWhatsapp, View overrides:

void WebWhatsapp::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (crashed_overlay_view_)
    crashed_overlay_view_->SetBoundsRect(gfx::Rect(size()));

  // In most cases, the holder is simply sized to fill this WebWhatsapp's bounds.
...
void WebWhatsapp::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details) {
  if (details.is_add)
    AttachWebContentsNativeView();
}

bool WebWhatsapp::SkipDefaultKeyEventProcessing(const ui::KeyEvent& event) {
...
bool WebWhatsapp::OnMousePressed(const ui::MouseEvent& event) {
  // A left-click within WebWhatsapp is a request to focus.  The area within the
...
void WebWhatsapp::OnFocus() {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->Focus();
}

void WebWhatsapp::AboutToRequestFocusFromTabTraversal(bool reverse) {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->FocusThroughTabTraversal(reverse);
}

void WebWhatsapp::AddedToWidget() {
  if (!web_contents())
    return;
...
gfx::NativeViewAccessible WebWhatsapp::GetNativeViewAccessible() {
  if (web_contents() && !web_contents()->IsCrashed()) {
...
void WebWhatsapp::OnAXModeAdded(ui::AXMode mode) {
  if (!GetWidget() || !web_contents())
    return;

  // Normally, it is set during AttachWebContentsNativeView when the WebWhatsapp is
  // created but this may not happen on some platforms as the accessible object
  // may not have been present when this WebWhatsapp was created. So, update it when
  // AX mode is added.
  UpdateNativeViewHostAccessibleParent(holder(), parent());
}

////////////////////////////////////////////////////////////////////////////////
// WebWhatsapp, content::WebContentsDelegate implementation:

////////////////////////////////////////////////////////////////////////////////
// WebWhatsapp, content::WebContentsObserver implementation:

void WebWhatsapp::RenderFrameCreated(content::RenderFrameHost* render_frame_host) {
...
void WebWhatsapp::RenderFrameDeleted(content::RenderFrameHost* render_frame_host) {
...
void WebWhatsapp::RenderFrameHostChanged(content::RenderFrameHost* old_host,
...
void WebWhatsapp::DidToggleFullscreenModeForTab(bool entered_fullscreen,
...
void WebWhatsapp::OnWebContentsFocused(
    content::RenderWidgetHost* render_widget_host) {
  RequestFocus();
}

void WebWhatsapp::AXTreeIDForMainFrameHasChanged() {
  NotifyAccessibilityWebContentsChanged();
}

void WebWhatsapp::WebContentsDestroyed() {
  SetWebContents(nullptr);
}

void WebWhatsapp::ResizeDueToAutoResize(content::WebContents* source,
...
void WebWhatsapp::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kWebView;
  // A webwhatsapp does not need an accessible name as the document title is
...
void WebWhatsapp::AttachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebWhatsapp::AttachWebContentsNativeView");
  // Prevents attachment if the WebWhatsapp isn't already in a Widget, or it's
  // already attached.
  if (!GetWidget() || !web_contents())
    return;
...
void WebWhatsapp::DetachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebWhatsapp::DetachWebContentsNativeView");
  if (web_contents()) {
    holder_->Detach();
  }
}

void WebWhatsapp::UpdateCrashedOverlayView() {
...
void WebWhatsapp::NotifyAccessibilityWebContentsChanged() {
...
std::unique_ptr<content::WebContents> WebWhatsapp::CreateWebContents(
...
void WebWhatsapp::SetUpNewMainFrame(content::RenderFrameHost* frame_host) {
...
void WebWhatsapp::LostMainFrame() {
  UpdateCrashedOverlayView();
  NotifyAccessibilityWebContentsChanged();
}

void WebWhatsapp::MaybeEnableAutoResize(content::RenderFrameHost* frame_host) {
  DCHECK(frame_host->IsRenderFrameLive());
  if (!max_size_.IsEmpty())
    frame_host->GetView()->EnableAutoResize(min_size_, max_size_);
}

BEGIN_METADATA(WebWhatsapp, View)
END_METADATA
