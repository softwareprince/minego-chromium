...
#ifndef UI_VIEWS_CONTROLS_WEBWHATSAPP_WEBWHATSAPP_H_
#define UI_VIEWS_CONTROLS_WEBWHATSAPP_WEBWHATSAPP_H_
...
#include "ui/views/controls/webWhatsapp/webWhatsapp_export.h"
#include "ui/views/metadata/view_factory.h"
#include "ui/views/view.h"

namespace views {

// Provides a view of a WebContents instance.  WebWhatsapp can be used standalone,
...
// for browser tabs).
//
// WebWhatsapp creates and owns a single child view, a NativeViewHost, which will
// hold and display the native view provided by a WebContents.
//
// EmbedFullscreenWidgetMode: When enabled, WebWhatsapp will observe for WebContents
...
// being screen-captured, the view will be centered within WebWhatsapp, sized to
// the aspect ratio of the capture video resolution, and scaling will be avoided
// whenever possible.
class WEBWHATSAPP_EXPORT WebWhatsapp : public View,
                               public content::WebContentsDelegate,
                               public content::WebContentsObserver,
                               public ui::AXModeObserver {
 public:
  METADATA_HEADER(WebWhatsapp);

  explicit WebWhatsapp(content::BrowserContext* browser_context = nullptr);

  WebWhatsapp(const WebWhatsapp&) = delete;
  WebWhatsapp& operator=(const WebWhatsapp&) = delete;

  ~WebWhatsapp() override;

  // This creates a WebContents if |browser_context_| has been set and there is
  // not yet a WebContents associated with this WebWhatsapp, otherwise it will
  // return a nullptr.
  content::WebContents* GetWebContents(
      base::Location creator_location = base::Location::Current());

  // WebWhatsapp does not assume ownership of WebContents set via this method, only
  ...
  //         WebWhatsapp's bounds change (default).
  ...
  // If enabled, this will make the WebWhatsapp's preferred size dependent on the
  // WebContents' size.
  ...
  class WEBWHATSAPP_EXPORT ScopedWebContentsCreatorForTesting {
   public:
 ...
 private:
  friend class WebWhatsappUnitTest;
  ...
  // Non-NULL if |web_contents()| was created and is owned by this WebWhatsapp.
  ...
  // Minimum and maximum sizes to determine WebWhatsapp bounds for auto-resizing.
  // Empty if auto resize is not enabled.
  gfx::Size min_size_;
  gfx::Size max_size_;
};

BEGIN_VIEW_BUILDER(WEBWHATSAPP_EXPORT, WebWhatsapp, View)
...
DEFINE_VIEW_BUILDER(WEBWHATSAPP_EXPORT, WebWhatsapp)

#endif  // UI_VIEWS_CONTROLS_WEBWHATSAPP_WEBWHATSAPP_H_
