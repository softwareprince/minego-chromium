...
class WebWhatsappTestWebContentsObserver : public content::WebContentsObserver {
 public:
  explicit WebWhatsappTestWebContentsObserver(content::WebContents* web_contents)
      : web_contents_(web_contents) {
    content::WebContentsObserver::Observe(web_contents);
  }

  WebWhatsappTestWebContentsObserver(const WebWhatsappTestWebContentsObserver&) =
      delete;
  WebWhatsappTestWebContentsObserver& operator=(
      const WebWhatsappTestWebContentsObserver&) = delete;

  ~WebWhatsappTestWebContentsObserver() override {
    if (web_contents_)
...
// Fakes the fullscreen browser state reported to WebContents and WebWhatsapp.
class WebWhatsappTestWebContentsDelegate : public content::WebContentsDelegate {
 public:
  WebWhatsappTestWebContentsDelegate() = default;

  WebWhatsappTestWebContentsDelegate(const WebWhatsappTestWebContentsDelegate&) =
      delete;
  WebWhatsappTestWebContentsDelegate& operator=(
      const WebWhatsappTestWebContentsDelegate&) = delete;

  ~WebWhatsappTestWebContentsDelegate() override = default;
...
// Provides functionality to test a WebWhatsapp.
class WebWhatsappUnitTest : public views::test::WidgetTest {
 public:
  WebWhatsappUnitTest()
      : views::test::WidgetTest(std::unique_ptr<base::test::TaskEnvironment>(
            std::make_unique<content::BrowserTaskEnvironment>())) {}

  WebWhatsappUnitTest(const WebWhatsappUnitTest&) = delete;
  WebWhatsappUnitTest& operator=(const WebWhatsappUnitTest&) = delete;

  ~WebWhatsappUnitTest() override = default;

  std::unique_ptr<content::WebContents> CreateWebContentsForWebWhatsapp(
  ...
  void SetUp() override {
    ...
    views::WebWhatsapp::WebContentsCreator creator = base::BindRepeating(
        &WebWhatsappUnitTest::CreateWebContentsForWebWhatsapp, base::Unretained(this));
    scoped_web_contents_creator_ =
        std::make_unique<views::WebWhatsapp::ScopedWebContentsCreatorForTesting>(
            creator);
    ...
    // Create a top level widget and add a child, and give it a WebWhatsapp as a
    // child.
    ...
    auto web_view = std::make_unique<WebWhatsapp>(browser_context_.get());
 ...
 protected:
  ...
  WebWhatsapp* web_view() const { return web_view_; }
 ...
 private:
  ...
  std::unique_ptr<views::WebWhatsapp::ScopedWebContentsCreatorForTesting>
      scoped_web_contents_creator_;

  raw_ptr<Widget> top_level_widget_ = nullptr;
  raw_ptr<WebWhatsapp> web_view_ = nullptr;
};

// Tests that attaching and detaching a WebContents to a WebWhatsapp makes the
// WebContents visible and hidden respectively.
TEST_F(WebWhatsappUnitTest, TestWebWhatsappAttachDetachWebContents) {
  // Case 1: Create a new WebContents and set it in the webwhatsapp via
  // SetWebContents. This should make the WebContents visible.
  const std::unique_ptr<content::WebContents> web_contents1(
      CreateWebContents());
  WebWhatsappTestWebContentsObserver observer1(web_contents1.get());
  ...
  const std::unique_ptr<content::WebContents> web_contents2(
      CreateWebContents());
  WebWhatsappTestWebContentsObserver observer2(web_contents2.get());
  ...
  // Case 3: Test that attaching to a hidden webwhatsapp does not show the web
  // contents.
...
#if defined(USE_AURA)
  // Case 4: Test that making the webwhatsapp visible when a window has an invisible
  ...
  // window visibility. However, we can still test that moving a visible WebWhatsapp
  // whose WebContents is not currently showing to a new, visible window will
  // show the WebContents. Simulate the "hide window with visible WebWhatsapp" step
  ...
  // supported use case. Instead, move the WebWhatsapp over.
  parent2->SetContentsView(web_view()->parent()->RemoveChildViewT(web_view()));
  EXPECT_EQ(3, observer1.shown_count());
  parent2->Close();
}

// Verifies that there is no crash in WebWhatsapp destructor
// if WebWhatsapp is already removed from Widget.
TEST_F(WebWhatsappUnitTest, DetachedWebWhatsappDestructor) {
  // Init WebWhatsapp with attached NativeView.
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  std::unique_ptr<WebWhatsapp> webwhatsapp(
      new WebWhatsapp(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  contents_view->AddChildView(webwhatsapp.get());
  webwhatsapp->SetWebContents(web_contents.get());

  // Remove WebWhatsapp from views hierarchy. NativeView should be detached
  // from Widget.
  contents_view->RemoveChildView(webwhatsapp.get());
  // Destroy WebWhatsapp. NativeView should be detached secondary.
...
TEST_F(WebWhatsappUnitTest, CrashedOverlayView) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());

  std::unique_ptr<WebWhatsapp> web_view(
      new WebWhatsapp(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  ...
  // Normally when a renderer crashes, the WebWhatsapp will learn about it
  // automatically via WebContentsObserver. Since this is a test
  // WebContents, simulate that by calling SetIsCrashed and then
  // explicitly calling RenderFrameDeleted on the WebWhatsapp to trigger it
...
TEST_F(WebWhatsappUnitTest, CrashedOverlayViewOwnedbyClient) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());
...
// Tests to make sure we can default construct the WebWhatsapp class and set the
// BrowserContext after construction.
TEST_F(WebWhatsappUnitTest, DefaultConstructability) {
  auto browser_context = std::make_unique<content::TestBrowserContext>();
  auto web_view = std::make_unique<WebWhatsapp>();

  // Test to make sure the WebWhatsapp returns a nullptr in the absence of an
  ...
  // WebWhatsapp should be able to create a WebContents object from the previously
  // set |browser_context|.
...
TEST_F(WebWhatsappUnitTest, ReparentingUpdatesParentAccessible) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  auto web_view = std::make_unique<WebWhatsapp>(web_contents->GetBrowserContext());
  web_view->SetWebContents(web_contents.get());

  WidgetAutoclosePtr widget_1(CreateTopLevelPlatformWidget());
  View* contents_view_1 = widget_1->GetContentsView();
  WebWhatsapp* added_web_view = contents_view_1->AddChildView(std::move(web_view));
...
// This tests that we don't crash if WebWhatsapp doesn't have a Widget or a
// Webcontents. https://crbug.com/1191999
TEST_F(WebWhatsappUnitTest, ChangeAXMode) {
  // Case 1: WebWhatsapp has a Widget and no WebContents.
  SetAXMode(ui::AXMode::kFirstModeFlag);

  // Case 2: WebWhatsapp has no Widget and a WebContents.
  View* contents_view = top_level_widget()->GetContentsView();
...
// Tests to make sure the WebWhatsapp clears away the reference to its hosted
// WebContents object when its deleted.
TEST_F(WebWhatsappUnitTest, WebWhatsappClearsWebContentsOnDestruction) {
