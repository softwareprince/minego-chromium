...
#ifndef UI_VIEWS_CONTROLS_WEBDISCORD_WEBDISCORD_H_
#define UI_VIEWS_CONTROLS_WEBDISCORD_WEBDISCORD_H_
...
#include "ui/views/controls/webDiscord/webDiscord_export.h"
#include "ui/views/metadata/view_factory.h"
#include "ui/views/view.h"

namespace views {

// Provides a view of a WebContents instance.  WebDiscord can be used standalone,
...
// for browser tabs).
//
// WebDiscord creates and owns a single child view, a NativeViewHost, which will
// hold and display the native view provided by a WebContents.
//
// EmbedFullscreenWidgetMode: When enabled, WebDiscord will observe for WebContents
...
// being screen-captured, the view will be centered within WebDiscord, sized to
// the aspect ratio of the capture video resolution, and scaling will be avoided
// whenever possible.
class WEBDISCORD_EXPORT WebDiscord : public View,
                               public content::WebContentsDelegate,
                               public content::WebContentsObserver,
                               public ui::AXModeObserver {
 public:
  METADATA_HEADER(WebDiscord);

  explicit WebDiscord(content::BrowserContext* browser_context = nullptr);

  WebDiscord(const WebDiscord&) = delete;
  WebDiscord& operator=(const WebDiscord&) = delete;

  ~WebDiscord() override;

  // This creates a WebContents if |browser_context_| has been set and there is
  // not yet a WebContents associated with this WebDiscord, otherwise it will
  // return a nullptr.
  content::WebContents* GetWebContents(
      base::Location creator_location = base::Location::Current());

  // WebDiscord does not assume ownership of WebContents set via this method, only
  ...
  //         WebDiscord's bounds change (default).
  ...
  // If enabled, this will make the WebDiscord's preferred size dependent on the
  // WebContents' size.
  ...
  class WEBDISCORD_EXPORT ScopedWebContentsCreatorForTesting {
   public:
 ...
 private:
  friend class WebDiscordUnitTest;
  ...
  // Non-NULL if |web_contents()| was created and is owned by this WebDiscord.
  ...
  // Minimum and maximum sizes to determine WebDiscord bounds for auto-resizing.
  // Empty if auto resize is not enabled.
  gfx::Size min_size_;
  gfx::Size max_size_;
};

BEGIN_VIEW_BUILDER(WEBDISCORD_EXPORT, WebDiscord, View)
...
DEFINE_VIEW_BUILDER(WEBDISCORD_EXPORT, WebDiscord)

#endif  // UI_VIEWS_CONTROLS_WEBDISCORD_WEBDISCORD_H_
