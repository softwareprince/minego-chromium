...
#include "ui/views/controls/webDiscord/webDiscord.h"
...
class WebDiscordTestWebContentsObserver : public content::WebContentsObserver {
 public:
  explicit WebDiscordTestWebContentsObserver(content::WebContents* web_contents)
      : web_contents_(web_contents) {
    content::WebContentsObserver::Observe(web_contents);
  }

  WebDiscordTestWebContentsObserver(const WebDiscordTestWebContentsObserver&) =
      delete;
  WebDiscordTestWebContentsObserver& operator=(
      const WebDiscordTestWebContentsObserver&) = delete;

  ~WebDiscordTestWebContentsObserver() override {
    if (web_contents_)
...
// Fakes the fullscreen browser state reported to WebContents and WebDiscord.
class WebDiscordTestWebContentsDelegate : public content::WebContentsDelegate {
 public:
  WebDiscordTestWebContentsDelegate() = default;

  WebDiscordTestWebContentsDelegate(const WebDiscordTestWebContentsDelegate&) =
      delete;
  WebDiscordTestWebContentsDelegate& operator=(
      const WebDiscordTestWebContentsDelegate&) = delete;

  ~WebDiscordTestWebContentsDelegate() override = default;
...
// Provides functionality to test a WebDiscord.
class WebDiscordUnitTest : public views::test::WidgetTest {
 public:
  WebDiscordUnitTest()
      : views::test::WidgetTest(std::unique_ptr<base::test::TaskEnvironment>(
            std::make_unique<content::BrowserTaskEnvironment>())) {}

  WebDiscordUnitTest(const WebDiscordUnitTest&) = delete;
  WebDiscordUnitTest& operator=(const WebDiscordUnitTest&) = delete;

  ~WebDiscordUnitTest() override = default;

  std::unique_ptr<content::WebContents> CreateWebContentsForWebDiscord(
  ...
  void SetUp() override {
    ...
    views::WebDiscord::WebContentsCreator creator = base::BindRepeating(
        &WebDiscordUnitTest::CreateWebContentsForWebDiscord, base::Unretained(this));
    scoped_web_contents_creator_ =
        std::make_unique<views::WebDiscord::ScopedWebContentsCreatorForTesting>(
            creator);
    ...
    // Create a top level widget and add a child, and give it a WebDiscord as a
    // child.
    ...
    auto web_view = std::make_unique<WebDiscord>(browser_context_.get());
 ...
 protected:
  ...
  WebDiscord* web_view() const { return web_view_; }
 ...
 private:
  ...
  std::unique_ptr<views::WebDiscord::ScopedWebContentsCreatorForTesting>
      scoped_web_contents_creator_;

  raw_ptr<Widget> top_level_widget_ = nullptr;
  raw_ptr<WebDiscord> web_view_ = nullptr;
};

// Tests that attaching and detaching a WebContents to a WebDiscord makes the
// WebContents visible and hidden respectively.
TEST_F(WebDiscordUnitTest, TestWebDiscordAttachDetachWebContents) {
  // Case 1: Create a new WebContents and set it in the webdiscord via
  // SetWebContents. This should make the WebContents visible.
  const std::unique_ptr<content::WebContents> web_contents1(
      CreateWebContents());
  WebDiscordTestWebContentsObserver observer1(web_contents1.get());
  ...
  const std::unique_ptr<content::WebContents> web_contents2(
      CreateWebContents());
  WebDiscordTestWebContentsObserver observer2(web_contents2.get());
  ...
  // Case 3: Test that attaching to a hidden webdiscord does not show the web
  // contents.
...
#if defined(USE_AURA)
  // Case 4: Test that making the webdiscord visible when a window has an invisible
  ...
  // window visibility. However, we can still test that moving a visible WebDiscord
  // whose WebContents is not currently showing to a new, visible window will
  // show the WebContents. Simulate the "hide window with visible WebDiscord" step
  ...
  // supported use case. Instead, move the WebDiscord over.
  parent2->SetContentsView(web_view()->parent()->RemoveChildViewT(web_view()));
  EXPECT_EQ(3, observer1.shown_count());
  parent2->Close();
}

// Verifies that there is no crash in WebDiscord destructor
// if WebDiscord is already removed from Widget.
TEST_F(WebDiscordUnitTest, DetachedWebDiscordDestructor) {
  // Init WebDiscord with attached NativeView.
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  std::unique_ptr<WebDiscord> webdiscord(
      new WebDiscord(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  contents_view->AddChildView(webdiscord.get());
  webdiscord->SetWebContents(web_contents.get());

  // Remove WebDiscord from views hierarchy. NativeView should be detached
  // from Widget.
  contents_view->RemoveChildView(webdiscord.get());
  // Destroy WebDiscord. NativeView should be detached secondary.
...
TEST_F(WebDiscordUnitTest, CrashedOverlayView) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());

  std::unique_ptr<WebDiscord> web_view(
      new WebDiscord(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  ...
  // Normally when a renderer crashes, the WebDiscord will learn about it
  // automatically via WebContentsObserver. Since this is a test
  // WebContents, simulate that by calling SetIsCrashed and then
  // explicitly calling RenderFrameDeleted on the WebDiscord to trigger it
...
TEST_F(WebDiscordUnitTest, CrashedOverlayViewOwnedbyClient) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());
...
// Tests to make sure we can default construct the WebDiscord class and set the
// BrowserContext after construction.
TEST_F(WebDiscordUnitTest, DefaultConstructability) {
  auto browser_context = std::make_unique<content::TestBrowserContext>();
  auto web_view = std::make_unique<WebDiscord>();

  // Test to make sure the WebDiscord returns a nullptr in the absence of an
  ...
  // WebDiscord should be able to create a WebContents object from the previously
  // set |browser_context|.
...
TEST_F(WebDiscordUnitTest, ReparentingUpdatesParentAccessible) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  auto web_view = std::make_unique<WebDiscord>(web_contents->GetBrowserContext());
  web_view->SetWebContents(web_contents.get());

  WidgetAutoclosePtr widget_1(CreateTopLevelPlatformWidget());
  View* contents_view_1 = widget_1->GetContentsView();
  WebDiscord* added_web_view = contents_view_1->AddChildView(std::move(web_view));
...
// This tests that we don't crash if WebDiscord doesn't have a Widget or a
// Webcontents. https://crbug.com/1191999
TEST_F(WebDiscordUnitTest, ChangeAXMode) {
  // Case 1: WebDiscord has a Widget and no WebContents.
  SetAXMode(ui::AXMode::kFirstModeFlag);

  // Case 2: WebDiscord has no Widget and a WebContents.
  View* contents_view = top_level_widget()->GetContentsView();
...
// Tests to make sure the WebDiscord clears away the reference to its hosted
// WebContents object when its deleted.
TEST_F(WebDiscordUnitTest, WebDiscordClearsWebContentsOnDestruction) {
