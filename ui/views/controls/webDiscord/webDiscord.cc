...
WebDiscord::WebContentsCreator* GetCreatorForTesting() {
  static base::NoDestructor<WebDiscord::WebContentsCreator> creator;
  return creator.get();
}

// Updates the parent accessible object on the NativeView. As WebDiscord overrides
...
WebDiscord::ScopedWebContentsCreatorForTesting::ScopedWebContentsCreatorForTesting(
    WebContentsCreator creator) {
  DCHECK(!*GetCreatorForTesting());
  *GetCreatorForTesting() = creator;
}

WebDiscord::ScopedWebContentsCreatorForTesting::
    ~ScopedWebContentsCreatorForTesting() {
  *GetCreatorForTesting() = WebDiscord::WebContentsCreator();
}

////////////////////////////////////////////////////////////////////////////////
// WebDiscord, public:

WebDiscord::WebDiscord(content::BrowserContext* browser_context) {
  set_suppress_default_focus_handling();
  ui::AXPlatformNode::AddAXModeObserver(this);
  SetBrowserContext(browser_context);
}

WebDiscord::~WebDiscord() {
  ui::AXPlatformNode::RemoveAXModeObserver(this);
  SetWebContents(nullptr);  // Make sure all necessary tear-down takes place.
}

content::WebContents* WebDiscord::GetWebContents(base::Location creator_location) {
  if (!web_contents()) {
    if (!browser_context_)
      return nullptr;
...
void WebDiscord::SetWebContents(content::WebContents* replacement) {
  TRACE_EVENT0("views", "WebDiscord::SetWebContents");
...
content::BrowserContext* WebDiscord::GetBrowserContext() {
  return browser_context_;
}

void WebDiscord::SetBrowserContext(content::BrowserContext* browser_context) {
  browser_context_ = browser_context;
}

void WebDiscord::LoadInitialURL(const GURL& url) {
...
void WebDiscord::SetFastResize(bool fast_resize) {
  holder_->set_fast_resize(fast_resize);
}

void WebDiscord::EnableSizingFromWebContents(const gfx::Size& min_size,
...
void WebDiscord::SetCrashedOverlayView(View* crashed_overlay_view) {
...
////////////////////////////////////////////////////////////////////////////////
// WebDiscord, View overrides:

void WebDiscord::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (crashed_overlay_view_)
    crashed_overlay_view_->SetBoundsRect(gfx::Rect(size()));

  // In most cases, the holder is simply sized to fill this WebDiscord's bounds.
...
void WebDiscord::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details) {
  if (details.is_add)
    AttachWebContentsNativeView();
}

bool WebDiscord::SkipDefaultKeyEventProcessing(const ui::KeyEvent& event) {
...
bool WebDiscord::OnMousePressed(const ui::MouseEvent& event) {
  // A left-click within WebDiscord is a request to focus.  The area within the
...
void WebDiscord::OnFocus() {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->Focus();
}

void WebDiscord::AboutToRequestFocusFromTabTraversal(bool reverse) {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->FocusThroughTabTraversal(reverse);
}

void WebDiscord::AddedToWidget() {
  if (!web_contents())
    return;
...
gfx::NativeViewAccessible WebDiscord::GetNativeViewAccessible() {
  if (web_contents() && !web_contents()->IsCrashed()) {
...
void WebDiscord::OnAXModeAdded(ui::AXMode mode) {
  if (!GetWidget() || !web_contents())
    return;

  // Normally, it is set during AttachWebContentsNativeView when the WebDiscord is
  // created but this may not happen on some platforms as the accessible object
  // may not have been present when this WebDiscord was created. So, update it when
  // AX mode is added.
  UpdateNativeViewHostAccessibleParent(holder(), parent());
}

////////////////////////////////////////////////////////////////////////////////
// WebDiscord, content::WebContentsDelegate implementation:

////////////////////////////////////////////////////////////////////////////////
// WebDiscord, content::WebContentsObserver implementation:

void WebDiscord::RenderFrameCreated(content::RenderFrameHost* render_frame_host) {
...
void WebDiscord::RenderFrameDeleted(content::RenderFrameHost* render_frame_host) {
...
void WebDiscord::RenderFrameHostChanged(content::RenderFrameHost* old_host,
...
void WebDiscord::DidToggleFullscreenModeForTab(bool entered_fullscreen,
...
void WebDiscord::OnWebContentsFocused(
    content::RenderWidgetHost* render_widget_host) {
  RequestFocus();
}

void WebDiscord::AXTreeIDForMainFrameHasChanged() {
  NotifyAccessibilityWebContentsChanged();
}

void WebDiscord::WebContentsDestroyed() {
  SetWebContents(nullptr);
}

void WebDiscord::ResizeDueToAutoResize(content::WebContents* source,
...
void WebDiscord::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kWebView;
  // A webdiscord does not need an accessible name as the document title is
...
void WebDiscord::AttachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebDiscord::AttachWebContentsNativeView");
  // Prevents attachment if the WebDiscord isn't already in a Widget, or it's
  // already attached.
  if (!GetWidget() || !web_contents())
    return;
...
void WebDiscord::DetachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebDiscord::DetachWebContentsNativeView");
  if (web_contents()) {
    holder_->Detach();
  }
}

void WebDiscord::UpdateCrashedOverlayView() {
...
void WebDiscord::NotifyAccessibilityWebContentsChanged() {
...
std::unique_ptr<content::WebContents> WebDiscord::CreateWebContents(
...
void WebDiscord::SetUpNewMainFrame(content::RenderFrameHost* frame_host) {
...
void WebDiscord::LostMainFrame() {
  UpdateCrashedOverlayView();
  NotifyAccessibilityWebContentsChanged();
}

void WebDiscord::MaybeEnableAutoResize(content::RenderFrameHost* frame_host) {
  DCHECK(frame_host->IsRenderFrameLive());
  if (!max_size_.IsEmpty())
    frame_host->GetView()->EnableAutoResize(min_size_, max_size_);
}

BEGIN_METADATA(WebDiscord, View)
END_METADATA
