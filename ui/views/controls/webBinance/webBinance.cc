...
WebBinance::WebContentsCreator* GetCreatorForTesting() {
  static base::NoDestructor<WebBinance::WebContentsCreator> creator;
  return creator.get();
}

// Updates the parent accessible object on the NativeView. As WebBinance overrides
...
WebBinance::ScopedWebContentsCreatorForTesting::ScopedWebContentsCreatorForTesting(
    WebContentsCreator creator) {
  DCHECK(!*GetCreatorForTesting());
  *GetCreatorForTesting() = creator;
}

WebBinance::ScopedWebContentsCreatorForTesting::
    ~ScopedWebContentsCreatorForTesting() {
  *GetCreatorForTesting() = WebBinance::WebContentsCreator();
}

////////////////////////////////////////////////////////////////////////////////
// WebBinance, public:

WebBinance::WebBinance(content::BrowserContext* browser_context) {
  set_suppress_default_focus_handling();
  ui::AXPlatformNode::AddAXModeObserver(this);
  SetBrowserContext(browser_context);
}

WebBinance::~WebBinance() {
  ui::AXPlatformNode::RemoveAXModeObserver(this);
  SetWebContents(nullptr);  // Make sure all necessary tear-down takes place.
}

content::WebContents* WebBinance::GetWebContents(base::Location creator_location) {
  if (!web_contents()) {
    if (!browser_context_)
      return nullptr;
...
void WebBinance::SetWebContents(content::WebContents* replacement) {
  TRACE_EVENT0("views", "WebBinance::SetWebContents");
...
content::BrowserContext* WebBinance::GetBrowserContext() {
  return browser_context_;
}

void WebBinance::SetBrowserContext(content::BrowserContext* browser_context) {
  browser_context_ = browser_context;
}

void WebBinance::LoadInitialURL(const GURL& url) {
...
void WebBinance::SetFastResize(bool fast_resize) {
  holder_->set_fast_resize(fast_resize);
}

void WebBinance::EnableSizingFromWebContents(const gfx::Size& min_size,
...
void WebBinance::SetCrashedOverlayView(View* crashed_overlay_view) {
...
////////////////////////////////////////////////////////////////////////////////
// WebBinance, View overrides:

void WebBinance::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (crashed_overlay_view_)
    crashed_overlay_view_->SetBoundsRect(gfx::Rect(size()));

  // In most cases, the holder is simply sized to fill this WebBinance's bounds.
...
void WebBinance::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details) {
  if (details.is_add)
    AttachWebContentsNativeView();
}

bool WebBinance::SkipDefaultKeyEventProcessing(const ui::KeyEvent& event) {
...
bool WebBinance::OnMousePressed(const ui::MouseEvent& event) {
  // A left-click within WebBinance is a request to focus.  The area within the
...
void WebBinance::OnFocus() {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->Focus();
}

void WebBinance::AboutToRequestFocusFromTabTraversal(bool reverse) {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->FocusThroughTabTraversal(reverse);
}

void WebBinance::AddedToWidget() {
  if (!web_contents())
    return;
...
gfx::NativeViewAccessible WebBinance::GetNativeViewAccessible() {
  if (web_contents() && !web_contents()->IsCrashed()) {
...
void WebBinance::OnAXModeAdded(ui::AXMode mode) {
  if (!GetWidget() || !web_contents())
    return;

  // Normally, it is set during AttachWebContentsNativeView when the WebBinance is
  // created but this may not happen on some platforms as the accessible object
  // may not have been present when this WebBinance was created. So, update it when
  // AX mode is added.
  UpdateNativeViewHostAccessibleParent(holder(), parent());
}

////////////////////////////////////////////////////////////////////////////////
// WebBinance, content::WebContentsDelegate implementation:

////////////////////////////////////////////////////////////////////////////////
// WebBinance, content::WebContentsObserver implementation:

void WebBinance::RenderFrameCreated(content::RenderFrameHost* render_frame_host) {
...
void WebBinance::RenderFrameDeleted(content::RenderFrameHost* render_frame_host) {
...
void WebBinance::RenderFrameHostChanged(content::RenderFrameHost* old_host,
...
void WebBinance::DidToggleFullscreenModeForTab(bool entered_fullscreen,
...
void WebBinance::OnWebContentsFocused(
    content::RenderWidgetHost* render_widget_host) {
  RequestFocus();
}

void WebBinance::AXTreeIDForMainFrameHasChanged() {
  NotifyAccessibilityWebContentsChanged();
}

void WebBinance::WebContentsDestroyed() {
  SetWebContents(nullptr);
}

void WebBinance::ResizeDueToAutoResize(content::WebContents* source,
...
void WebBinance::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kWebView;
  // A webbinance does not need an accessible name as the document title is
...
void WebBinance::AttachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebBinance::AttachWebContentsNativeView");
  // Prevents attachment if the WebBinance isn't already in a Widget, or it's
  // already attached.
  if (!GetWidget() || !web_contents())
    return;
...
void WebBinance::DetachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebBinance::DetachWebContentsNativeView");
  if (web_contents()) {
    holder_->Detach();
  }
}

void WebBinance::UpdateCrashedOverlayView() {
...
void WebBinance::NotifyAccessibilityWebContentsChanged() {
...
std::unique_ptr<content::WebContents> WebBinance::CreateWebContents(
...
void WebBinance::SetUpNewMainFrame(content::RenderFrameHost* frame_host) {
...
void WebBinance::LostMainFrame() {
  UpdateCrashedOverlayView();
  NotifyAccessibilityWebContentsChanged();
}

void WebBinance::MaybeEnableAutoResize(content::RenderFrameHost* frame_host) {
  DCHECK(frame_host->IsRenderFrameLive());
  if (!max_size_.IsEmpty())
    frame_host->GetView()->EnableAutoResize(min_size_, max_size_);
}

BEGIN_METADATA(WebBinance, View)
END_METADATA
