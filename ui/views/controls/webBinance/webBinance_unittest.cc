...
#include "ui/views/controls/webBinance/webBinance.h"
...
class WebBinanceTestWebContentsObserver : public content::WebContentsObserver {
 public:
  explicit WebBinanceTestWebContentsObserver(content::WebContents* web_contents)
      : web_contents_(web_contents) {
    content::WebContentsObserver::Observe(web_contents);
  }

  WebBinanceTestWebContentsObserver(const WebBinanceTestWebContentsObserver&) =
      delete;
  WebBinanceTestWebContentsObserver& operator=(
      const WebBinanceTestWebContentsObserver&) = delete;

  ~WebBinanceTestWebContentsObserver() override {
    if (web_contents_)
...
// Fakes the fullscreen browser state reported to WebContents and WebBinance.
class WebBinanceTestWebContentsDelegate : public content::WebContentsDelegate {
 public:
  WebBinanceTestWebContentsDelegate() = default;

  WebBinanceTestWebContentsDelegate(const WebBinanceTestWebContentsDelegate&) =
      delete;
  WebBinanceTestWebContentsDelegate& operator=(
      const WebBinanceTestWebContentsDelegate&) = delete;

  ~WebBinanceTestWebContentsDelegate() override = default;
...
// Provides functionality to test a WebBinance.
class WebBinanceUnitTest : public views::test::WidgetTest {
 public:
  WebBinanceUnitTest()
      : views::test::WidgetTest(std::unique_ptr<base::test::TaskEnvironment>(
            std::make_unique<content::BrowserTaskEnvironment>())) {}

  WebBinanceUnitTest(const WebBinanceUnitTest&) = delete;
  WebBinanceUnitTest& operator=(const WebBinanceUnitTest&) = delete;

  ~WebBinanceUnitTest() override = default;

  std::unique_ptr<content::WebContents> CreateWebContentsForWebBinance(
  ...
  void SetUp() override {
    ...
    views::WebBinance::WebContentsCreator creator = base::BindRepeating(
        &WebBinanceUnitTest::CreateWebContentsForWebBinance, base::Unretained(this));
    scoped_web_contents_creator_ =
        std::make_unique<views::WebBinance::ScopedWebContentsCreatorForTesting>(
            creator);
    ...
    // Create a top level widget and add a child, and give it a WebBinance as a
    // child.
    ...
    auto web_view = std::make_unique<WebBinance>(browser_context_.get());
 ...
 protected:
  ...
  WebBinance* web_view() const { return web_view_; }
 ...
 private:
  ...
  std::unique_ptr<views::WebBinance::ScopedWebContentsCreatorForTesting>
      scoped_web_contents_creator_;

  raw_ptr<Widget> top_level_widget_ = nullptr;
  raw_ptr<WebBinance> web_view_ = nullptr;
};

// Tests that attaching and detaching a WebContents to a WebBinance makes the
// WebContents visible and hidden respectively.
TEST_F(WebBinanceUnitTest, TestWebBinanceAttachDetachWebContents) {
  // Case 1: Create a new WebContents and set it in the webbinance via
  // SetWebContents. This should make the WebContents visible.
  const std::unique_ptr<content::WebContents> web_contents1(
      CreateWebContents());
  WebBinanceTestWebContentsObserver observer1(web_contents1.get());
  ...
  const std::unique_ptr<content::WebContents> web_contents2(
      CreateWebContents());
  WebBinanceTestWebContentsObserver observer2(web_contents2.get());
  ...
  // Case 3: Test that attaching to a hidden webbinance does not show the web
  // contents.
...
#if defined(USE_AURA)
  // Case 4: Test that making the webbinance visible when a window has an invisible
  ...
  // window visibility. However, we can still test that moving a visible WebBinance
  // whose WebContents is not currently showing to a new, visible window will
  // show the WebContents. Simulate the "hide window with visible WebBinance" step
  ...
  // supported use case. Instead, move the WebBinance over.
  parent2->SetContentsView(web_view()->parent()->RemoveChildViewT(web_view()));
  EXPECT_EQ(3, observer1.shown_count());
  parent2->Close();
}

// Verifies that there is no crash in WebBinance destructor
// if WebBinance is already removed from Widget.
TEST_F(WebBinanceUnitTest, DetachedWebBinanceDestructor) {
  // Init WebBinance with attached NativeView.
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  std::unique_ptr<WebBinance> webbinance(
      new WebBinance(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  contents_view->AddChildView(webbinance.get());
  webbinance->SetWebContents(web_contents.get());

  // Remove WebBinance from views hierarchy. NativeView should be detached
  // from Widget.
  contents_view->RemoveChildView(webbinance.get());
  // Destroy WebBinance. NativeView should be detached secondary.
...
TEST_F(WebBinanceUnitTest, CrashedOverlayView) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());

  std::unique_ptr<WebBinance> web_view(
      new WebBinance(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  ...
  // Normally when a renderer crashes, the WebBinance will learn about it
  // automatically via WebContentsObserver. Since this is a test
  // WebContents, simulate that by calling SetIsCrashed and then
  // explicitly calling RenderFrameDeleted on the WebBinance to trigger it
...
TEST_F(WebBinanceUnitTest, CrashedOverlayViewOwnedbyClient) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());
...
// Tests to make sure we can default construct the WebBinance class and set the
// BrowserContext after construction.
TEST_F(WebBinanceUnitTest, DefaultConstructability) {
  auto browser_context = std::make_unique<content::TestBrowserContext>();
  auto web_view = std::make_unique<WebBinance>();

  // Test to make sure the WebBinance returns a nullptr in the absence of an
  ...
  // WebBinance should be able to create a WebContents object from the previously
  // set |browser_context|.
...
TEST_F(WebBinanceUnitTest, ReparentingUpdatesParentAccessible) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  auto web_view = std::make_unique<WebBinance>(web_contents->GetBrowserContext());
  web_view->SetWebContents(web_contents.get());

  WidgetAutoclosePtr widget_1(CreateTopLevelPlatformWidget());
  View* contents_view_1 = widget_1->GetContentsView();
  WebBinance* added_web_view = contents_view_1->AddChildView(std::move(web_view));
...
// This tests that we don't crash if WebBinance doesn't have a Widget or a
// Webcontents. https://crbug.com/1191999
TEST_F(WebBinanceUnitTest, ChangeAXMode) {
  // Case 1: WebBinance has a Widget and no WebContents.
  SetAXMode(ui::AXMode::kFirstModeFlag);

  // Case 2: WebBinance has no Widget and a WebContents.
  View* contents_view = top_level_widget()->GetContentsView();
...
// Tests to make sure the WebBinance clears away the reference to its hosted
// WebContents object when its deleted.
TEST_F(WebBinanceUnitTest, WebBinanceClearsWebContentsOnDestruction) {
