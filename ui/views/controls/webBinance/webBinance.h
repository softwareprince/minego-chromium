...
#ifndef UI_VIEWS_CONTROLS_WEBBINANCE_WEBBINANCE_H_
#define UI_VIEWS_CONTROLS_WEBBINANCE_WEBBINANCE_H_
...
#include "ui/views/controls/webBinance/webBinance_export.h"
#include "ui/views/metadata/view_factory.h"
#include "ui/views/view.h"

namespace views {

// Provides a view of a WebContents instance.  WebBinance can be used standalone,
...
// for browser tabs).
//
// WebBinance creates and owns a single child view, a NativeViewHost, which will
// hold and display the native view provided by a WebContents.
//
// EmbedFullscreenWidgetMode: When enabled, WebBinance will observe for WebContents
...
// being screen-captured, the view will be centered within WebBinance, sized to
// the aspect ratio of the capture video resolution, and scaling will be avoided
// whenever possible.
class WEBBINANCE_EXPORT WebBinance : public View,
                               public content::WebContentsDelegate,
                               public content::WebContentsObserver,
                               public ui::AXModeObserver {
 public:
  METADATA_HEADER(WebBinance);

  explicit WebBinance(content::BrowserContext* browser_context = nullptr);

  WebBinance(const WebBinance&) = delete;
  WebBinance& operator=(const WebBinance&) = delete;

  ~WebBinance() override;

  // This creates a WebContents if |browser_context_| has been set and there is
  // not yet a WebContents associated with this WebBinance, otherwise it will
  // return a nullptr.
  content::WebContents* GetWebContents(
      base::Location creator_location = base::Location::Current());

  // WebBinance does not assume ownership of WebContents set via this method, only
  ...
  //         WebBinance's bounds change (default).
  ...
  // If enabled, this will make the WebBinance's preferred size dependent on the
  // WebContents' size.
  ...
  class WEBBINANCE_EXPORT ScopedWebContentsCreatorForTesting {
   public:
 ...
 private:
  friend class WebBinanceUnitTest;
  ...
  // Non-NULL if |web_contents()| was created and is owned by this WebBinance.
  ...
  // Minimum and maximum sizes to determine WebBinance bounds for auto-resizing.
  // Empty if auto resize is not enabled.
  gfx::Size min_size_;
  gfx::Size max_size_;
};

BEGIN_VIEW_BUILDER(WEBBINANCE_EXPORT, WebBinance, View)
...
DEFINE_VIEW_BUILDER(WEBBINANCE_EXPORT, WebBinance)

#endif  // UI_VIEWS_CONTROLS_WEBBINANCE_WEBBINANCE_H_
