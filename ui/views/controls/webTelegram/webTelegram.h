...
#ifndef UI_VIEWS_CONTROLS_WEBTELEGRAM_WEBTELEGRAM_H_
#define UI_VIEWS_CONTROLS_WEBTELEGRAM_WEBTELEGRAM_H_
...
#include "ui/views/controls/webTelegram/webTelegram_export.h"
#include "ui/views/metadata/view_factory.h"
#include "ui/views/view.h"

namespace views {

// Provides a view of a WebContents instance.  WebTelegram can be used standalone,
...
// for browser tabs).
//
// WebTelegram creates and owns a single child view, a NativeViewHost, which will
// hold and display the native view provided by a WebContents.
//
// EmbedFullscreenWidgetMode: When enabled, WebTelegram will observe for WebContents
...
// being screen-captured, the view will be centered within WebTelegram, sized to
// the aspect ratio of the capture video resolution, and scaling will be avoided
// whenever possible.
class WEBTELEGRAM_EXPORT WebTelegram : public View,
                               public content::WebContentsDelegate,
                               public content::WebContentsObserver,
                               public ui::AXModeObserver {
 public:
  METADATA_HEADER(WebTelegram);

  explicit WebTelegram(content::BrowserContext* browser_context = nullptr);

  WebTelegram(const WebTelegram&) = delete;
  WebTelegram& operator=(const WebTelegram&) = delete;

  ~WebTelegram() override;

  // This creates a WebContents if |browser_context_| has been set and there is
  // not yet a WebContents associated with this WebTelegram, otherwise it will
  // return a nullptr.
  content::WebContents* GetWebContents(
      base::Location creator_location = base::Location::Current());

  // WebTelegram does not assume ownership of WebContents set via this method, only
  ...
  //         WebTelegram's bounds change (default).
  ...
  // If enabled, this will make the WebTelegram's preferred size dependent on the
  // WebContents' size.
  ...
  class WEBTELEGRAM_EXPORT ScopedWebContentsCreatorForTesting {
   public:
 ...
 private:
  friend class WebTelegramUnitTest;
  ...
  // Non-NULL if |web_contents()| was created and is owned by this WebTelegram.
  ...
  // Minimum and maximum sizes to determine WebTelegram bounds for auto-resizing.
  // Empty if auto resize is not enabled.
  gfx::Size min_size_;
  gfx::Size max_size_;
};

BEGIN_VIEW_BUILDER(WEBTELEGRAM_EXPORT, WebTelegram, View)
...
DEFINE_VIEW_BUILDER(WEBTELEGRAM_EXPORT, WebTelegram)

#endif  // UI_VIEWS_CONTROLS_WEBTELEGRAM_WEBTELEGRAM_H_
