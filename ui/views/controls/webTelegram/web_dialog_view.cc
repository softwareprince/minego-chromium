...
#include "ui/views/controls/webTelegram/web_dialog_view.h"
...
ObservableWebTelegram::ObservableWebTelegram(content::BrowserContext* browser_context,
                                     WebDialogDelegate* delegate)
    : WebTelegram(browser_context), delegate_(delegate) {}

ObservableWebTelegram::~ObservableWebTelegram() = default;

void ObservableWebTelegram::DidFinishLoad(
...
void ObservableWebTelegram::ResetDelegate() {
  delegate_ = nullptr;
}

BEGIN_METADATA(ObservableWebTelegram, WebTelegram)
END_METADATA
...
WebDialogView::WebDialogView(content::BrowserContext* context,
    ...
    : ClientView(nullptr, nullptr),
      ...
      delegate_(delegate),
      web_view_(new ObservableWebTelegram(context, delegate)) {
...
ADD_READONLY_PROPERTY_METADATA(ObservableWebTelegram*, WebTelegram);
END_METADATA
