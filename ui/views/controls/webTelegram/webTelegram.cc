...
WebTelegram::WebContentsCreator* GetCreatorForTesting() {
  static base::NoDestructor<WebTelegram::WebContentsCreator> creator;
  return creator.get();
}

// Updates the parent accessible object on the NativeView. As WebTelegram overrides
...
WebTelegram::ScopedWebContentsCreatorForTesting::ScopedWebContentsCreatorForTesting(
    WebContentsCreator creator) {
  DCHECK(!*GetCreatorForTesting());
  *GetCreatorForTesting() = creator;
}

WebTelegram::ScopedWebContentsCreatorForTesting::
    ~ScopedWebContentsCreatorForTesting() {
  *GetCreatorForTesting() = WebTelegram::WebContentsCreator();
}

////////////////////////////////////////////////////////////////////////////////
// WebTelegram, public:

WebTelegram::WebTelegram(content::BrowserContext* browser_context) {
...
content::WebContents* WebTelegram::GetWebContents(base::Location creator_location) {
  if (!web_contents()) {
    if (!browser_context_)
      return nullptr;
...
void WebTelegram::SetWebContents(content::WebContents* replacement) {
  TRACE_EVENT0("views", "WebTelegram::SetWebContents");
...
content::BrowserContext* WebTelegram::GetBrowserContext() {
  return browser_context_;
}

void WebTelegram::SetBrowserContext(content::BrowserContext* browser_context) {
  browser_context_ = browser_context;
}

void WebTelegram::LoadInitialURL(const GURL& url) {
...
void WebTelegram::SetFastResize(bool fast_resize) {
  holder_->set_fast_resize(fast_resize);
}

void WebTelegram::EnableSizingFromWebContents(const gfx::Size& min_size,
...
void WebTelegram::SetCrashedOverlayView(View* crashed_overlay_view) {
...
////////////////////////////////////////////////////////////////////////////////
// WebTelegram, View overrides:

void WebTelegram::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (crashed_overlay_view_)
    crashed_overlay_view_->SetBoundsRect(gfx::Rect(size()));

  // In most cases, the holder is simply sized to fill this WebTelegram's bounds.
...
void WebTelegram::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details) {
  if (details.is_add)
    AttachWebContentsNativeView();
}

bool WebTelegram::SkipDefaultKeyEventProcessing(const ui::KeyEvent& event) {
...
bool WebTelegram::OnMousePressed(const ui::MouseEvent& event) {
  // A left-click within WebTelegram is a request to focus.  The area within the
...
void WebTelegram::OnFocus() {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->Focus();
}

void WebTelegram::AboutToRequestFocusFromTabTraversal(bool reverse) {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->FocusThroughTabTraversal(reverse);
}

void WebTelegram::AddedToWidget() {
  if (!web_contents())
    return;
...
gfx::NativeViewAccessible WebTelegram::GetNativeViewAccessible() {
  if (web_contents() && !web_contents()->IsCrashed()) {
...
void WebTelegram::OnAXModeAdded(ui::AXMode mode) {
  if (!GetWidget() || !web_contents())
    return;

  // Normally, it is set during AttachWebContentsNativeView when the WebTelegram is
  // created but this may not happen on some platforms as the accessible object
  // may not have been present when this WebTelegram was created. So, update it when
  // AX mode is added.
  UpdateNativeViewHostAccessibleParent(holder(), parent());
}

////////////////////////////////////////////////////////////////////////////////
// WebTelegram, content::WebContentsDelegate implementation:

////////////////////////////////////////////////////////////////////////////////
// WebTelegram, content::WebContentsObserver implementation:

void WebTelegram::RenderFrameCreated(content::RenderFrameHost* render_frame_host) {
...
void WebTelegram::RenderFrameDeleted(content::RenderFrameHost* render_frame_host) {
...
void WebTelegram::RenderFrameHostChanged(content::RenderFrameHost* old_host,
...
void WebTelegram::DidToggleFullscreenModeForTab(bool entered_fullscreen,
...
void WebTelegram::OnWebContentsFocused(
    content::RenderWidgetHost* render_widget_host) {
  RequestFocus();
}

void WebTelegram::AXTreeIDForMainFrameHasChanged() {
  NotifyAccessibilityWebContentsChanged();
}

void WebTelegram::WebContentsDestroyed() {
  SetWebContents(nullptr);
}

void WebTelegram::ResizeDueToAutoResize(content::WebContents* source,
...
void WebTelegram::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kWebView;
  // A webtelegram does not need an accessible name as the document title is
...
void WebTelegram::AttachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebTelegram::AttachWebContentsNativeView");
  // Prevents attachment if the WebTelegram isn't already in a Widget, or it's
  // already attached.
  if (!GetWidget() || !web_contents())
    return;
...
void WebTelegram::DetachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebTelegram::DetachWebContentsNativeView");
  if (web_contents()) {
    holder_->Detach();
  }
}

void WebTelegram::UpdateCrashedOverlayView() {
...
void WebTelegram::NotifyAccessibilityWebContentsChanged() {
...
std::unique_ptr<content::WebContents> WebTelegram::CreateWebContents(
...
void WebTelegram::SetUpNewMainFrame(content::RenderFrameHost* frame_host) {
...
void WebTelegram::LostMainFrame() {
  UpdateCrashedOverlayView();
  NotifyAccessibilityWebContentsChanged();
}

void WebTelegram::MaybeEnableAutoResize(content::RenderFrameHost* frame_host) {
  DCHECK(frame_host->IsRenderFrameLive());
  if (!max_size_.IsEmpty())
    frame_host->GetView()->EnableAutoResize(min_size_, max_size_);
}

BEGIN_METADATA(WebTelegram, View)
END_METADATA
