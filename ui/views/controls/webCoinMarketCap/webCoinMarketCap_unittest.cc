...
class WebCoinMarketCapTestWebContentsObserver : public content::WebContentsObserver {
 public:
  explicit WebCoinMarketCapTestWebContentsObserver(content::WebContents* web_contents)
      : web_contents_(web_contents) {
    content::WebContentsObserver::Observe(web_contents);
  }

  WebCoinMarketCapTestWebContentsObserver(const WebCoinMarketCapTestWebContentsObserver&) =
      delete;
  WebCoinMarketCapTestWebContentsObserver& operator=(
      const WebCoinMarketCapTestWebContentsObserver&) = delete;

  ~WebCoinMarketCapTestWebContentsObserver() override {
    if (web_contents_)
...
// Fakes the fullscreen browser state reported to WebContents and WebCoinMarketCap.
class WebCoinMarketCapTestWebContentsDelegate : public content::WebContentsDelegate {
 public:
  WebCoinMarketCapTestWebContentsDelegate() = default;

  WebCoinMarketCapTestWebContentsDelegate(const WebCoinMarketCapTestWebContentsDelegate&) =
      delete;
  WebCoinMarketCapTestWebContentsDelegate& operator=(
      const WebCoinMarketCapTestWebContentsDelegate&) = delete;

  ~WebCoinMarketCapTestWebContentsDelegate() override = default;
...
// Provides functionality to test a WebCoinMarketCap.
class WebCoinMarketCapUnitTest : public views::test::WidgetTest {
 public:
  WebCoinMarketCapUnitTest()
      : views::test::WidgetTest(std::unique_ptr<base::test::TaskEnvironment>(
            std::make_unique<content::BrowserTaskEnvironment>())) {}

  WebCoinMarketCapUnitTest(const WebCoinMarketCapUnitTest&) = delete;
  WebCoinMarketCapUnitTest& operator=(const WebCoinMarketCapUnitTest&) = delete;

  ~WebCoinMarketCapUnitTest() override = default;

  std::unique_ptr<content::WebContents> CreateWebContentsForWebCoinMarketCap(
  ...
  void SetUp() override {
    ...
    views::WebCoinMarketCap::WebContentsCreator creator = base::BindRepeating(
        &WebCoinMarketCapUnitTest::CreateWebContentsForWebCoinMarketCap, base::Unretained(this));
    scoped_web_contents_creator_ =
        std::make_unique<views::WebCoinMarketCap::ScopedWebContentsCreatorForTesting>(
            creator);
    ...
    // Create a top level widget and add a child, and give it a WebCoinMarketCap as a
    // child.
    ...
    auto web_view = std::make_unique<WebCoinMarketCap>(browser_context_.get());
 ...
 protected:
  ...
  WebCoinMarketCap* web_view() const { return web_view_; }
 ...
 private:
  ...
  std::unique_ptr<views::WebCoinMarketCap::ScopedWebContentsCreatorForTesting>
      scoped_web_contents_creator_;

  raw_ptr<Widget> top_level_widget_ = nullptr;
  raw_ptr<WebCoinMarketCap> web_view_ = nullptr;
};

// Tests that attaching and detaching a WebContents to a WebCoinMarketCap makes the
// WebContents visible and hidden respectively.
TEST_F(WebCoinMarketCapUnitTest, TestWebCoinMarketCapAttachDetachWebContents) {
  // Case 1: Create a new WebContents and set it in the webcoinmarketcap via
  // SetWebContents. This should make the WebContents visible.
  const std::unique_ptr<content::WebContents> web_contents1(
      CreateWebContents());
  WebCoinMarketCapTestWebContentsObserver observer1(web_contents1.get());
  ...
  const std::unique_ptr<content::WebContents> web_contents2(
      CreateWebContents());
  WebCoinMarketCapTestWebContentsObserver observer2(web_contents2.get());
  ...
  // Case 3: Test that attaching to a hidden webcoinmarketcap does not show the web
  // contents.
...
#if defined(USE_AURA)
  // Case 4: Test that making the webcoinmarketcap visible when a window has an invisible
  ...
  // window visibility. However, we can still test that moving a visible WebCoinMarketCap
  // whose WebContents is not currently showing to a new, visible window will
  // show the WebContents. Simulate the "hide window with visible WebCoinMarketCap" step
  ...
  // supported use case. Instead, move the WebCoinMarketCap over.
  parent2->SetContentsView(web_view()->parent()->RemoveChildViewT(web_view()));
  EXPECT_EQ(3, observer1.shown_count());
  parent2->Close();
}

// Verifies that there is no crash in WebCoinMarketCap destructor
// if WebCoinMarketCap is already removed from Widget.
TEST_F(WebCoinMarketCapUnitTest, DetachedWebCoinMarketCapDestructor) {
  // Init WebCoinMarketCap with attached NativeView.
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  std::unique_ptr<WebCoinMarketCap> webcoinmarketcap(
      new WebCoinMarketCap(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  contents_view->AddChildView(webcoinmarketcap.get());
  webcoinmarketcap->SetWebContents(web_contents.get());

  // Remove WebCoinMarketCap from views hierarchy. NativeView should be detached
  // from Widget.
  contents_view->RemoveChildView(webcoinmarketcap.get());
  // Destroy WebCoinMarketCap. NativeView should be detached secondary.
  // There should be no crash.
  webcoinmarketcap.reset();
}

// Test that the specified crashed overlay view is shown when a WebContents
// is in a crashed state.
TEST_F(WebCoinMarketCapUnitTest, CrashedOverlayView) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());

  std::unique_ptr<WebCoinMarketCap> web_view(
      new WebCoinMarketCap(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  ...
  // Normally when a renderer crashes, the WebCoinMarketCap will learn about it
  // automatically via WebContentsObserver. Since this is a test
  // WebContents, simulate that by calling SetIsCrashed and then
  // explicitly calling RenderFrameDeleted on the WebCoinMarketCap to trigger it
...
TEST_F(WebCoinMarketCapUnitTest, CrashedOverlayViewOwnedbyClient) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());
...
// Tests to make sure we can default construct the WebCoinMarketCap class and set the
// BrowserContext after construction.
TEST_F(WebCoinMarketCapUnitTest, DefaultConstructability) {
  auto browser_context = std::make_unique<content::TestBrowserContext>();
  auto web_view = std::make_unique<WebCoinMarketCap>();

  // Test to make sure the WebCoinMarketCap returns a nullptr in the absence of an
  ...
  // WebCoinMarketCap should be able to create a WebContents object from the previously
  // set |browser_context|.
...
TEST_F(WebCoinMarketCapUnitTest, ReparentingUpdatesParentAccessible) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  auto web_view = std::make_unique<WebCoinMarketCap>(web_contents->GetBrowserContext());
  web_view->SetWebContents(web_contents.get());

  WidgetAutoclosePtr widget_1(CreateTopLevelPlatformWidget());
  View* contents_view_1 = widget_1->GetContentsView();
  WebCoinMarketCap* added_web_view = contents_view_1->AddChildView(std::move(web_view));
...
// This tests that we don't crash if WebCoinMarketCap doesn't have a Widget or a
// Webcontents. https://crbug.com/1191999
TEST_F(WebCoinMarketCapUnitTest, ChangeAXMode) {
  // Case 1: WebCoinMarketCap has a Widget and no WebContents.
  SetAXMode(ui::AXMode::kFirstModeFlag);

  // Case 2: WebCoinMarketCap has no Widget and a WebContents.
  View* contents_view = top_level_widget()->GetContentsView();
...
// Tests to make sure the WebCoinMarketCap clears away the reference to its hosted
// WebContents object when its deleted.
TEST_F(WebCoinMarketCapUnitTest, WebCoinMarketCapClearsWebContentsOnDestruction) {
