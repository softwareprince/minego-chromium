...
class WebNaeemTestWebContentsObserver : public content::WebContentsObserver {
 public:
  explicit WebNaeemTestWebContentsObserver(content::WebContents* web_contents)
      : web_contents_(web_contents) {
    content::WebContentsObserver::Observe(web_contents);
  }

  WebNaeemTestWebContentsObserver(const WebNaeemTestWebContentsObserver&) =
      delete;
  WebNaeemTestWebContentsObserver& operator=(
      const WebNaeemTestWebContentsObserver&) = delete;

  ~WebNaeemTestWebContentsObserver() override {
    if (web_contents_)
...
// Fakes the fullscreen browser state reported to WebContents and WebNaeem.
class WebNaeemTestWebContentsDelegate : public content::WebContentsDelegate {
 public:
  WebNaeemTestWebContentsDelegate() = default;

  WebNaeemTestWebContentsDelegate(const WebNaeemTestWebContentsDelegate&) =
      delete;
  WebNaeemTestWebContentsDelegate& operator=(
      const WebNaeemTestWebContentsDelegate&) = delete;

  ~WebNaeemTestWebContentsDelegate() override = default;
...
// Provides functionality to test a WebNaeem.
class WebNaeemUnitTest : public views::test::WidgetTest {
 public:
  WebNaeemUnitTest()
      : views::test::WidgetTest(std::unique_ptr<base::test::TaskEnvironment>(
            std::make_unique<content::BrowserTaskEnvironment>())) {}

  WebNaeemUnitTest(const WebNaeemUnitTest&) = delete;
  WebNaeemUnitTest& operator=(const WebNaeemUnitTest&) = delete;

  ~WebNaeemUnitTest() override = default;

  std::unique_ptr<content::WebContents> CreateWebContentsForWebNaeem(
  ...
  void SetUp() override {
    ...
    views::WebNaeem::WebContentsCreator creator = base::BindRepeating(
        &WebNaeemUnitTest::CreateWebContentsForWebNaeem, base::Unretained(this));
    scoped_web_contents_creator_ =
        std::make_unique<views::WebNaeem::ScopedWebContentsCreatorForTesting>(
            creator);
    ...
    // Create a top level widget and add a child, and give it a WebNaeem as a
    // child.
    ...
    auto web_view = std::make_unique<WebNaeem>(browser_context_.get());
 ...
 protected:
  ...
  WebNaeem* web_view() const { return web_view_; }
 ...
 private:
  ...
  std::unique_ptr<views::WebNaeem::ScopedWebContentsCreatorForTesting>
      scoped_web_contents_creator_;

  raw_ptr<Widget> top_level_widget_ = nullptr;
  raw_ptr<WebNaeem> web_view_ = nullptr;
};

// Tests that attaching and detaching a WebContents to a WebNaeem makes the
// WebContents visible and hidden respectively.
TEST_F(WebNaeemUnitTest, TestWebNaeemAttachDetachWebContents) {
  // Case 1: Create a new WebContents and set it in the webnaeem via
  // SetWebContents. This should make the WebContents visible.
  const std::unique_ptr<content::WebContents> web_contents1(
      CreateWebContents());
  WebNaeemTestWebContentsObserver observer1(web_contents1.get());
  ...
  const std::unique_ptr<content::WebContents> web_contents2(
      CreateWebContents());
  WebNaeemTestWebContentsObserver observer2(web_contents2.get());
  ...
  // Case 3: Test that attaching to a hidden webnaeem does not show the web
  // contents.
...
#if defined(USE_AURA)
  // Case 4: Test that making the webnaeem visible when a window has an invisible
  ...
  // window visibility. However, we can still test that moving a visible WebNaeem
  // whose WebContents is not currently showing to a new, visible window will
  // show the WebContents. Simulate the "hide window with visible WebNaeem" step
  ...
  // supported use case. Instead, move the WebNaeem over.
  parent2->SetContentsView(web_view()->parent()->RemoveChildViewT(web_view()));
  EXPECT_EQ(3, observer1.shown_count());
  parent2->Close();
}

// Verifies that there is no crash in WebNaeem destructor
// if WebNaeem is already removed from Widget.
TEST_F(WebNaeemUnitTest, DetachedWebNaeemDestructor) {
  // Init WebNaeem with attached NativeView.
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  std::unique_ptr<WebNaeem> webnaeem(
      new WebNaeem(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  contents_view->AddChildView(webnaeem.get());
  webnaeem->SetWebContents(web_contents.get());

  // Remove WebNaeem from views hierarchy. NativeView should be detached
  // from Widget.
  contents_view->RemoveChildView(webnaeem.get());
  // Destroy WebNaeem. NativeView should be detached secondary.
...
TEST_F(WebNaeemUnitTest, CrashedOverlayView) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());

  std::unique_ptr<WebNaeem> web_view(
      new WebNaeem(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  ...
  // Normally when a renderer crashes, the WebNaeem will learn about it
  // automatically via WebContentsObserver. Since this is a test
  // WebContents, simulate that by calling SetIsCrashed and then
  // explicitly calling RenderFrameDeleted on the WebNaeem to trigger it
...
TEST_F(WebNaeemUnitTest, CrashedOverlayViewOwnedbyClient) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());
...
// Tests to make sure we can default construct the WebNaeem class and set the
// BrowserContext after construction.
TEST_F(WebNaeemUnitTest, DefaultConstructability) {
  auto browser_context = std::make_unique<content::TestBrowserContext>();
  auto web_view = std::make_unique<WebNaeem>();

  // Test to make sure the WebNaeem returns a nullptr in the absence of an
  ...
  // WebNaeem should be able to create a WebContents object from the previously
  // set |browser_context|.
...
TEST_F(WebNaeemUnitTest, ReparentingUpdatesParentAccessible) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  auto web_view = std::make_unique<WebNaeem>(web_contents->GetBrowserContext());
  web_view->SetWebContents(web_contents.get());

  WidgetAutoclosePtr widget_1(CreateTopLevelPlatformWidget());
  View* contents_view_1 = widget_1->GetContentsView();
  WebNaeem* added_web_view = contents_view_1->AddChildView(std::move(web_view));
...
// This tests that we don't crash if WebNaeem doesn't have a Widget or a
// Webcontents. https://crbug.com/1191999
TEST_F(WebNaeemUnitTest, ChangeAXMode) {
  // Case 1: WebNaeem has a Widget and no WebContents.
  SetAXMode(ui::AXMode::kFirstModeFlag);

  // Case 2: WebNaeem has no Widget and a WebContents.
  View* contents_view = top_level_widget()->GetContentsView();
...
// Tests to make sure the WebNaeem clears away the reference to its hosted
// WebContents object when its deleted.
TEST_F(WebNaeemUnitTest, WebNaeemClearsWebContentsOnDestruction) {
