...
WebNaeem::WebContentsCreator* GetCreatorForTesting() {
  static base::NoDestructor<WebNaeem::WebContentsCreator> creator;
  return creator.get();
}

// Updates the parent accessible object on the NativeView. As WebNaeem overrides
...
WebNaeem::ScopedWebContentsCreatorForTesting::ScopedWebContentsCreatorForTesting(
    WebContentsCreator creator) {
  DCHECK(!*GetCreatorForTesting());
  *GetCreatorForTesting() = creator;
}

WebNaeem::ScopedWebContentsCreatorForTesting::
    ~ScopedWebContentsCreatorForTesting() {
  *GetCreatorForTesting() = WebNaeem::WebContentsCreator();
}

////////////////////////////////////////////////////////////////////////////////
// WebNaeem, public:

WebNaeem::WebNaeem(content::BrowserContext* browser_context) {
...
content::WebContents* WebNaeem::GetWebContents(base::Location creator_location) {
  if (!web_contents()) {
    if (!browser_context_)
      return nullptr;
...
void WebNaeem::SetWebContents(content::WebContents* replacement) {
  TRACE_EVENT0("views", "WebNaeem::SetWebContents");
...
content::BrowserContext* WebNaeem::GetBrowserContext() {
  return browser_context_;
}

void WebNaeem::SetBrowserContext(content::BrowserContext* browser_context) {
  browser_context_ = browser_context;
}

void WebNaeem::LoadInitialURL(const GURL& url) {
...
void WebNaeem::SetFastResize(bool fast_resize) {
  holder_->set_fast_resize(fast_resize);
}

void WebNaeem::EnableSizingFromWebContents(const gfx::Size& min_size,
...
void WebNaeem::SetCrashedOverlayView(View* crashed_overlay_view) {
...
////////////////////////////////////////////////////////////////////////////////
// WebNaeem, View overrides:

void WebNaeem::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (crashed_overlay_view_)
    crashed_overlay_view_->SetBoundsRect(gfx::Rect(size()));

  // In most cases, the holder is simply sized to fill this WebNaeem's bounds.
...
void WebNaeem::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details) {
  if (details.is_add)
    AttachWebContentsNativeView();
}

bool WebNaeem::SkipDefaultKeyEventProcessing(const ui::KeyEvent& event) {
...
bool WebNaeem::OnMousePressed(const ui::MouseEvent& event) {
  // A left-click within WebNaeem is a request to focus.  The area within the
...
void WebNaeem::OnFocus() {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->Focus();
}

void WebNaeem::AboutToRequestFocusFromTabTraversal(bool reverse) {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->FocusThroughTabTraversal(reverse);
}

void WebNaeem::AddedToWidget() {
  if (!web_contents())
    return;
...
gfx::NativeViewAccessible WebNaeem::GetNativeViewAccessible() {
  if (web_contents() && !web_contents()->IsCrashed()) {
...
void WebNaeem::OnAXModeAdded(ui::AXMode mode) {
  if (!GetWidget() || !web_contents())
    return;

  // Normally, it is set during AttachWebContentsNativeView when the WebNaeem is
  // created but this may not happen on some platforms as the accessible object
  // may not have been present when this WebNaeem was created. So, update it when
  // AX mode is added.
  UpdateNativeViewHostAccessibleParent(holder(), parent());
}

////////////////////////////////////////////////////////////////////////////////
// WebNaeem, content::WebContentsDelegate implementation:

////////////////////////////////////////////////////////////////////////////////
// WebNaeem, content::WebContentsObserver implementation:

void WebNaeem::RenderFrameCreated(content::RenderFrameHost* render_frame_host) {
...
void WebNaeem::RenderFrameDeleted(content::RenderFrameHost* render_frame_host) {
...
void WebNaeem::RenderFrameHostChanged(content::RenderFrameHost* old_host,
...
void WebNaeem::DidToggleFullscreenModeForTab(bool entered_fullscreen,
...
void WebNaeem::OnWebContentsFocused(
    content::RenderWidgetHost* render_widget_host) {
  RequestFocus();
}

void WebNaeem::AXTreeIDForMainFrameHasChanged() {
  NotifyAccessibilityWebContentsChanged();
}

void WebNaeem::WebContentsDestroyed() {
  SetWebContents(nullptr);
}

void WebNaeem::ResizeDueToAutoResize(content::WebContents* source,
...
void WebNaeem::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kWebView;
  // A webnaeem does not need an accessible name as the document title is
...
void WebNaeem::AttachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebNaeem::AttachWebContentsNativeView");
  // Prevents attachment if the WebNaeem isn't already in a Widget, or it's
  // already attached.
  if (!GetWidget() || !web_contents())
    return;
...
void WebNaeem::DetachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebNaeem::DetachWebContentsNativeView");
  if (web_contents()) {
    holder_->Detach();
  }
}

void WebNaeem::UpdateCrashedOverlayView() {
...
void WebNaeem::NotifyAccessibilityWebContentsChanged() {
...
std::unique_ptr<content::WebContents> WebNaeem::CreateWebContents(
...
void WebNaeem::SetUpNewMainFrame(content::RenderFrameHost* frame_host) {
...
void WebNaeem::LostMainFrame() {
  UpdateCrashedOverlayView();
  NotifyAccessibilityWebContentsChanged();
}

void WebNaeem::MaybeEnableAutoResize(content::RenderFrameHost* frame_host) {
  DCHECK(frame_host->IsRenderFrameLive());
  if (!max_size_.IsEmpty())
    frame_host->GetView()->EnableAutoResize(min_size_, max_size_);
}

BEGIN_METADATA(WebNaeem, View)
END_METADATA
