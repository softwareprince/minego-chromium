...
#ifndef UI_VIEWS_CONTROLS_WEBNAEEM_WEBNAEEM_H_
#define UI_VIEWS_CONTROLS_WEBNAEEM_WEBNAEEM_H_
...
// Provides a view of a WebContents instance.  WebNaeem can be used standalone,
...
// for browser tabs).
//
// WebNaeem creates and owns a single child view, a NativeViewHost, which will
// hold and display the native view provided by a WebContents.
//
// EmbedFullscreenWidgetMode: When enabled, WebNaeem will observe for WebContents
...
// being screen-captured, the view will be centered within WebNaeem, sized to
// the aspect ratio of the capture video resolution, and scaling will be avoided
// whenever possible.
class WEBNAEEM_EXPORT WebNaeem : public View,
                               public content::WebContentsDelegate,
                               public content::WebContentsObserver,
                               public ui::AXModeObserver {
 public:
  METADATA_HEADER(WebNaeem);

  explicit WebNaeem(content::BrowserContext* browser_context = nullptr);

  WebNaeem(const WebNaeem&) = delete;
  WebNaeem& operator=(const WebNaeem&) = delete;

  ~WebNaeem() override;

  // This creates a WebContents if |browser_context_| has been set and there is
  // not yet a WebContents associated with this WebNaeem, otherwise it will
  // return a nullptr.
  content::WebContents* GetWebContents(
      base::Location creator_location = base::Location::Current());

  // WebNaeem does not assume ownership of WebContents set via this method, only
  ...
  //         WebNaeem's bounds change (default).
  ...
  // If enabled, this will make the WebNaeem's preferred size dependent on the
  // WebContents' size.
  ...
  class WEBNAEEM_EXPORT ScopedWebContentsCreatorForTesting {
   public:
 ...
 private:
  ...
  // Non-NULL if |web_contents()| was created and is owned by this WebNaeem.
  ...
  // Minimum and maximum sizes to determine WebNaeem bounds for auto-resizing.
  // Empty if auto resize is not enabled.
  gfx::Size min_size_;
  gfx::Size max_size_;
};

BEGIN_VIEW_BUILDER(WEBNAEEM_EXPORT, WebNaeem, View)
...
DEFINE_VIEW_BUILDER(WEBNAEEM_EXPORT, WebNaeem)

#endif  // UI_VIEWS_CONTROLS_WEBNAEEM_WEBNAEEM_H_
