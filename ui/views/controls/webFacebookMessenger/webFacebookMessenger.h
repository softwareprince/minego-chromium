...
#ifndef UI_VIEWS_CONTROLS_WEBFACEBOOKMESSENGER_WEBFACEBOOKMESSENGER_H_
#define UI_VIEWS_CONTROLS_WEBFACEBOOKMESSENGER_WEBFACEBOOKMESSENGER_H_
...
#include "ui/views/controls/webFacebookMessenger/webFacebookMessenger_export.h"
#include "ui/views/metadata/view_factory.h"
#include "ui/views/view.h"

namespace views {

// Provides a view of a WebContents instance.  WebFacebookMessenger can be used standalone,
...
// for browser tabs).
//
// WebFacebookMessenger creates and owns a single child view, a NativeViewHost, which will
// hold and display the native view provided by a WebContents.
//
// EmbedFullscreenWidgetMode: When enabled, WebFacebookMessenger will observe for WebContents
...
// being screen-captured, the view will be centered within WebFacebookMessenger, sized to
// the aspect ratio of the capture video resolution, and scaling will be avoided
// whenever possible.
class WEBFACEBOOKMESSENGER_EXPORT WebFacebookMessenger : public View,
                               public content::WebContentsDelegate,
                               public content::WebContentsObserver,
                               public ui::AXModeObserver {
 public:
  METADATA_HEADER(WebFacebookMessenger);

  explicit WebFacebookMessenger(content::BrowserContext* browser_context = nullptr);

  WebFacebookMessenger(const WebFacebookMessenger&) = delete;
  WebFacebookMessenger& operator=(const WebFacebookMessenger&) = delete;

  ~WebFacebookMessenger() override;

  // This creates a WebContents if |browser_context_| has been set and there is
  // not yet a WebContents associated with this WebFacebookMessenger, otherwise it will
  // return a nullptr.
  content::WebContents* GetWebContents(
      base::Location creator_location = base::Location::Current());

  // WebFacebookMessenger does not assume ownership of WebContents set via this method, only
  ...
  //         WebFacebookMessenger's bounds change (default).
  ...
  // If enabled, this will make the WebFacebookMessenger's preferred size dependent on the
  // WebContents' size.
  ...
  class WEBFACEBOOKMESSENGER_EXPORT ScopedWebContentsCreatorForTesting {
   public:
 ...
 private:
  friend class WebFacebookMessengerUnitTest;
  ...
  // Non-NULL if |web_contents()| was created and is owned by this WebFacebookMessenger.
  ...
  // Minimum and maximum sizes to determine WebFacebookMessenger bounds for auto-resizing.
  // Empty if auto resize is not enabled.
  gfx::Size min_size_;
  gfx::Size max_size_;
};

BEGIN_VIEW_BUILDER(WEBFACEBOOKMESSENGER_EXPORT, WebFacebookMessenger, View)
...
DEFINE_VIEW_BUILDER(WEBFACEBOOKMESSENGER_EXPORT, WebFacebookMessenger)

#endif  // UI_VIEWS_CONTROLS_WEBFACEBOOKMESSENGER_WEBFACEBOOKMESSENGER_H_
