...
WebFacebookMessenger::WebContentsCreator* GetCreatorForTesting() {
  static base::NoDestructor<WebFacebookMessenger::WebContentsCreator> creator;
  return creator.get();
}

// Updates the parent accessible object on the NativeView. As WebFacebookMessenger overrides
...
WebFacebookMessenger::ScopedWebContentsCreatorForTesting::ScopedWebContentsCreatorForTesting(
    WebContentsCreator creator) {
  DCHECK(!*GetCreatorForTesting());
  *GetCreatorForTesting() = creator;
}

WebFacebookMessenger::ScopedWebContentsCreatorForTesting::
    ~ScopedWebContentsCreatorForTesting() {
  *GetCreatorForTesting() = WebFacebookMessenger::WebContentsCreator();
}

////////////////////////////////////////////////////////////////////////////////
// WebFacebookMessenger, public:

WebFacebookMessenger::WebFacebookMessenger(content::BrowserContext* browser_context) {
  set_suppress_default_focus_handling();
  ui::AXPlatformNode::AddAXModeObserver(this);
  SetBrowserContext(browser_context);
}

WebFacebookMessenger::~WebFacebookMessenger() {
  ui::AXPlatformNode::RemoveAXModeObserver(this);
  SetWebContents(nullptr);  // Make sure all necessary tear-down takes place.
}

content::WebContents* WebFacebookMessenger::GetWebContents(base::Location creator_location) {
  if (!web_contents()) {
    if (!browser_context_)
      return nullptr;
...
void WebFacebookMessenger::SetWebContents(content::WebContents* replacement) {
  TRACE_EVENT0("views", "WebFacebookMessenger::SetWebContents");
...
content::BrowserContext* WebFacebookMessenger::GetBrowserContext() {
  return browser_context_;
}

void WebFacebookMessenger::SetBrowserContext(content::BrowserContext* browser_context) {
  browser_context_ = browser_context;
}

void WebFacebookMessenger::LoadInitialURL(const GURL& url) {
...
void WebFacebookMessenger::SetFastResize(bool fast_resize) {
  holder_->set_fast_resize(fast_resize);
}

void WebFacebookMessenger::EnableSizingFromWebContents(const gfx::Size& min_size,
...
void WebFacebookMessenger::SetCrashedOverlayView(View* crashed_overlay_view) {
...
////////////////////////////////////////////////////////////////////////////////
// WebFacebookMessenger, View overrides:

void WebFacebookMessenger::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (crashed_overlay_view_)
    crashed_overlay_view_->SetBoundsRect(gfx::Rect(size()));

  // In most cases, the holder is simply sized to fill this WebFacebookMessenger's bounds.
...
void WebFacebookMessenger::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details) {
  if (details.is_add)
    AttachWebContentsNativeView();
}

bool WebFacebookMessenger::SkipDefaultKeyEventProcessing(const ui::KeyEvent& event) {
...
bool WebFacebookMessenger::OnMousePressed(const ui::MouseEvent& event) {
  // A left-click within WebFacebookMessenger is a request to focus.  The area within the
...
void WebFacebookMessenger::OnFocus() {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->Focus();
}

void WebFacebookMessenger::AboutToRequestFocusFromTabTraversal(bool reverse) {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->FocusThroughTabTraversal(reverse);
}

void WebFacebookMessenger::AddedToWidget() {
  if (!web_contents())
    return;
  ...
  // WebFacebookMessenger.
  if (holder_->native_view())
    UpdateNativeViewHostAccessibleParent(holder_, parent());
}

gfx::NativeViewAccessible WebFacebookMessenger::GetNativeViewAccessible() {
  if (web_contents() && !web_contents()->IsCrashed()) {
...
void WebFacebookMessenger::OnAXModeAdded(ui::AXMode mode) {
  if (!GetWidget() || !web_contents())
    return;

  // Normally, it is set during AttachWebContentsNativeView when the WebFacebookMessenger is
  // created but this may not happen on some platforms as the accessible object
  // may not have been present when this WebFacebookMessenger was created. So, update it when
  // AX mode is added.
  UpdateNativeViewHostAccessibleParent(holder(), parent());
}

////////////////////////////////////////////////////////////////////////////////
// WebFacebookMessenger, content::WebContentsDelegate implementation:

////////////////////////////////////////////////////////////////////////////////
// WebFacebookMessenger, content::WebContentsObserver implementation:

void WebFacebookMessenger::RenderFrameCreated(content::RenderFrameHost* render_frame_host) {
...
void WebFacebookMessenger::RenderFrameDeleted(content::RenderFrameHost* render_frame_host) {
...
void WebFacebookMessenger::RenderFrameHostChanged(content::RenderFrameHost* old_host,
...
void WebFacebookMessenger::DidToggleFullscreenModeForTab(bool entered_fullscreen,
...
void WebFacebookMessenger::OnWebContentsFocused(
    content::RenderWidgetHost* render_widget_host) {
  RequestFocus();
}

void WebFacebookMessenger::AXTreeIDForMainFrameHasChanged() {
  NotifyAccessibilityWebContentsChanged();
}

void WebFacebookMessenger::WebContentsDestroyed() {
  SetWebContents(nullptr);
}

void WebFacebookMessenger::ResizeDueToAutoResize(content::WebContents* source,
...
void WebFacebookMessenger::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kWebView;
  // A webfacebookmessenger does not need an accessible name as the document title is
...
//a: command + shift + .
////////////////////////////////////////////////////////////////////////////////
// WebFacebookMessenger, private:

void WebFacebookMessenger::AttachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebFacebookMessenger::AttachWebContentsNativeView");
  // Prevents attachment if the WebFacebookMessenger isn't already in a Widget, or it's
  // already attached.
  if (!GetWidget() || !web_contents())
    return;
...
void WebFacebookMessenger::DetachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebFacebookMessenger::DetachWebContentsNativeView");
  if (web_contents()) {
    holder_->Detach();
  }
}

void WebFacebookMessenger::UpdateCrashedOverlayView() {
...
void WebFacebookMessenger::NotifyAccessibilityWebContentsChanged() {
...
std::unique_ptr<content::WebContents> WebFacebookMessenger::CreateWebContents(
...
void WebFacebookMessenger::SetUpNewMainFrame(content::RenderFrameHost* frame_host) {
...
void WebFacebookMessenger::LostMainFrame() {
  UpdateCrashedOverlayView();
  NotifyAccessibilityWebContentsChanged();
}

void WebFacebookMessenger::MaybeEnableAutoResize(content::RenderFrameHost* frame_host) {
  DCHECK(frame_host->IsRenderFrameLive());
  if (!max_size_.IsEmpty())
    frame_host->GetView()->EnableAutoResize(min_size_, max_size_);
}

BEGIN_METADATA(WebFacebookMessenger, View)
END_METADATA
