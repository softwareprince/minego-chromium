...
#include "ui/views/controls/webFacebookMessenger/webFacebookMessenger.h"
...
class WebFacebookMessengerTestWebContentsObserver : public content::WebContentsObserver {
 public:
  explicit WebFacebookMessengerTestWebContentsObserver(content::WebContents* web_contents)
      : web_contents_(web_contents) {
    content::WebContentsObserver::Observe(web_contents);
  }

  WebFacebookMessengerTestWebContentsObserver(const WebFacebookMessengerTestWebContentsObserver&) =
      delete;
  WebFacebookMessengerTestWebContentsObserver& operator=(
      const WebFacebookMessengerTestWebContentsObserver&) = delete;

  ~WebFacebookMessengerTestWebContentsObserver() override {
    if (web_contents_)
...
// Fakes the fullscreen browser state reported to WebContents and WebFacebookMessenger.
class WebFacebookMessengerTestWebContentsDelegate : public content::WebContentsDelegate {
 public:
  WebFacebookMessengerTestWebContentsDelegate() = default;

  WebFacebookMessengerTestWebContentsDelegate(const WebFacebookMessengerTestWebContentsDelegate&) =
      delete;
  WebFacebookMessengerTestWebContentsDelegate& operator=(
      const WebFacebookMessengerTestWebContentsDelegate&) = delete;

  ~WebFacebookMessengerTestWebContentsDelegate() override = default;
...
// Provides functionality to test a WebFacebookMessenger.
class WebFacebookMessengerUnitTest : public views::test::WidgetTest {
 public:
  WebFacebookMessengerUnitTest()
      : views::test::WidgetTest(std::unique_ptr<base::test::TaskEnvironment>(
            std::make_unique<content::BrowserTaskEnvironment>())) {}

  WebFacebookMessengerUnitTest(const WebFacebookMessengerUnitTest&) = delete;
  WebFacebookMessengerUnitTest& operator=(const WebFacebookMessengerUnitTest&) = delete;

  ~WebFacebookMessengerUnitTest() override = default;

  std::unique_ptr<content::WebContents> CreateWebContentsForWebFacebookMessenger(
  ...
  void SetUp() override {
    ...
    views::WebFacebookMessenger::WebContentsCreator creator = base::BindRepeating(
        &WebFacebookMessengerUnitTest::CreateWebContentsForWebFacebookMessenger, base::Unretained(this));
    scoped_web_contents_creator_ =
        std::make_unique<views::WebFacebookMessenger::ScopedWebContentsCreatorForTesting>(
            creator);
    ...
    // Create a top level widget and add a child, and give it a WebFacebookMessenger as a
    // child.
    ...
    auto web_view = std::make_unique<WebFacebookMessenger>(browser_context_.get());
 ...
 protected:
  ...
  WebFacebookMessenger* web_view() const { return web_view_; }
 ...
 private:
  ...
  std::unique_ptr<views::WebFacebookMessenger::ScopedWebContentsCreatorForTesting>
      scoped_web_contents_creator_;

  raw_ptr<Widget> top_level_widget_ = nullptr;
  raw_ptr<WebFacebookMessenger> web_view_ = nullptr;
};

// Tests that attaching and detaching a WebContents to a WebFacebookMessenger makes the
// WebContents visible and hidden respectively.
TEST_F(WebFacebookMessengerUnitTest, TestWebFacebookMessengerAttachDetachWebContents) {
  // Case 1: Create a new WebContents and set it in the webfacebookmessenger via
  // SetWebContents. This should make the WebContents visible.
  const std::unique_ptr<content::WebContents> web_contents1(
      CreateWebContents());
  WebFacebookMessengerTestWebContentsObserver observer1(web_contents1.get());
  ...
  const std::unique_ptr<content::WebContents> web_contents2(
      CreateWebContents());
  WebFacebookMessengerTestWebContentsObserver observer2(web_contents2.get());
  ...
  // Case 3: Test that attaching to a hidden webfacebookmessenger does not show the web
  // contents.
...
#if defined(USE_AURA)
  // Case 4: Test that making the webfacebookmessenger visible when a window has an invisible
  ...
  // window visibility. However, we can still test that moving a visible WebFacebookMessenger
  // whose WebContents is not currently showing to a new, visible window will
  // show the WebContents. Simulate the "hide window with visible WebFacebookMessenger" step
  ...
  // supported use case. Instead, move the WebFacebookMessenger over.
  parent2->SetContentsView(web_view()->parent()->RemoveChildViewT(web_view()));
  EXPECT_EQ(3, observer1.shown_count());
  parent2->Close();
}

// Verifies that there is no crash in WebFacebookMessenger destructor
// if WebFacebookMessenger is already removed from Widget.
TEST_F(WebFacebookMessengerUnitTest, DetachedWebFacebookMessengerDestructor) {
  // Init WebFacebookMessenger with attached NativeView.
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  std::unique_ptr<WebFacebookMessenger> webfacebookmessenger(
      new WebFacebookMessenger(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  contents_view->AddChildView(webfacebookmessenger.get());
  webfacebookmessenger->SetWebContents(web_contents.get());

  // Remove WebFacebookMessenger from views hierarchy. NativeView should be detached
  // from Widget.
  contents_view->RemoveChildView(webfacebookmessenger.get());
  // Destroy WebFacebookMessenger. NativeView should be detached secondary.
  // There should be no crash.
  webfacebookmessenger.reset();
}

// Test that the specified crashed overlay view is shown when a WebContents
// is in a crashed state.
TEST_F(WebFacebookMessengerUnitTest, CrashedOverlayView) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());

  std::unique_ptr<WebFacebookMessenger> web_view(
      new WebFacebookMessenger(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  ...
  // Normally when a renderer crashes, the WebFacebookMessenger will learn about it
  // automatically via WebContentsObserver. Since this is a test
  // WebContents, simulate that by calling SetIsCrashed and then
  // explicitly calling RenderFrameDeleted on the WebFacebookMessenger to trigger it
...
TEST_F(WebFacebookMessengerUnitTest, CrashedOverlayViewOwnedbyClient) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());
...
// Tests to make sure we can default construct the WebFacebookMessenger class and set the
// BrowserContext after construction.
TEST_F(WebFacebookMessengerUnitTest, DefaultConstructability) {
  auto browser_context = std::make_unique<content::TestBrowserContext>();
  auto web_view = std::make_unique<WebFacebookMessenger>();

  // Test to make sure the WebFacebookMessenger returns a nullptr in the absence of an
  ...
  // WebFacebookMessenger should be able to create a WebContents object from the previously
  // set |browser_context|.
...
TEST_F(WebFacebookMessengerUnitTest, ReparentingUpdatesParentAccessible) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  auto web_view = std::make_unique<WebFacebookMessenger>(web_contents->GetBrowserContext());
  web_view->SetWebContents(web_contents.get());

  WidgetAutoclosePtr widget_1(CreateTopLevelPlatformWidget());
  View* contents_view_1 = widget_1->GetContentsView();
  WebFacebookMessenger* added_web_view = contents_view_1->AddChildView(std::move(web_view));
...
// This tests that we don't crash if WebFacebookMessenger doesn't have a Widget or a
// Webcontents. https://crbug.com/1191999
TEST_F(WebFacebookMessengerUnitTest, ChangeAXMode) {
  // Case 1: WebFacebookMessenger has a Widget and no WebContents.
  SetAXMode(ui::AXMode::kFirstModeFlag);

  // Case 2: WebFacebookMessenger has no Widget and a WebContents.
  View* contents_view = top_level_widget()->GetContentsView();
...
// Tests to make sure the WebFacebookMessenger clears away the reference to its hosted
// WebContents object when its deleted.
TEST_F(WebFacebookMessengerUnitTest, WebFacebookMessengerClearsWebContentsOnDestruction) {
