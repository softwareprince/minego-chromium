...
#ifndef UI_VIEWS_CONTROLS_WEBFACEBOOK_WEBFACEBOOK_H_
#define UI_VIEWS_CONTROLS_WEBFACEBOOK_WEBFACEBOOK_H_
...
#include "ui/views/controls/webFacebook/webFacebook_export.h"
#include "ui/views/metadata/view_factory.h"
#include "ui/views/view.h"

namespace views {

// Provides a view of a WebContents instance.  WebFacebook can be used standalone,
...
// for browser tabs).
//
// WebFacebook creates and owns a single child view, a NativeViewHost, which will
// hold and display the native view provided by a WebContents.
//
// EmbedFullscreenWidgetMode: When enabled, WebFacebook will observe for WebContents
...
// being screen-captured, the view will be centered within WebFacebook, sized to
// the aspect ratio of the capture video resolution, and scaling will be avoided
// whenever possible.
class WEBFACEBOOK_EXPORT WebFacebook : public View,
                               public content::WebContentsDelegate,
                               public content::WebContentsObserver,
                               public ui::AXModeObserver {
 public:
  METADATA_HEADER(WebFacebook);

  explicit WebFacebook(content::BrowserContext* browser_context = nullptr);

  WebFacebook(const WebFacebook&) = delete;
  WebFacebook& operator=(const WebFacebook&) = delete;

  ~WebFacebook() override;

  // This creates a WebContents if |browser_context_| has been set and there is
  // not yet a WebContents associated with this WebFacebook, otherwise it will
  // return a nullptr.
  content::WebContents* GetWebContents(
      base::Location creator_location = base::Location::Current());

  // WebFacebook does not assume ownership of WebContents set via this method, only
  ...
  //         WebFacebook's bounds change (default).
  ...
  // If enabled, this will make the WebFacebook's preferred size dependent on the
  // WebContents' size.
  ...
  class WEBFACEBOOK_EXPORT ScopedWebContentsCreatorForTesting {
   public:
 ...
 private:
  friend class WebFacebookUnitTest;
  ...
  // Non-NULL if |web_contents()| was created and is owned by this WebFacebook.
  ...
  // Minimum and maximum sizes to determine WebFacebook bounds for auto-resizing.
  // Empty if auto resize is not enabled.
  gfx::Size min_size_;
  gfx::Size max_size_;
};

BEGIN_VIEW_BUILDER(WEBFACEBOOK_EXPORT, WebFacebook, View)
...
DEFINE_VIEW_BUILDER(WEBFACEBOOK_EXPORT, WebFacebook)

#endif  // UI_VIEWS_CONTROLS_WEBFACEBOOK_WEBFACEBOOK_H_
