...
WebFacebook::WebContentsCreator* GetCreatorForTesting() {
  static base::NoDestructor<WebFacebook::WebContentsCreator> creator;
  return creator.get();
}

// Updates the parent accessible object on the NativeView. As WebFacebook overrides
...
WebFacebook::ScopedWebContentsCreatorForTesting::ScopedWebContentsCreatorForTesting(
    WebContentsCreator creator) {
  DCHECK(!*GetCreatorForTesting());
  *GetCreatorForTesting() = creator;
}

WebFacebook::ScopedWebContentsCreatorForTesting::
    ~ScopedWebContentsCreatorForTesting() {
  *GetCreatorForTesting() = WebFacebook::WebContentsCreator();
}

////////////////////////////////////////////////////////////////////////////////
// WebFacebook, public:

WebFacebook::WebFacebook(content::BrowserContext* browser_context) {
...
content::WebContents* WebFacebook::GetWebContents(base::Location creator_location) {
  if (!web_contents()) {
    if (!browser_context_)
      return nullptr;
...
void WebFacebook::SetWebContents(content::WebContents* replacement) {
  TRACE_EVENT0("views", "WebFacebook::SetWebContents");
...
content::BrowserContext* WebFacebook::GetBrowserContext() {
  return browser_context_;
}

void WebFacebook::SetBrowserContext(content::BrowserContext* browser_context) {
  browser_context_ = browser_context;
}

void WebFacebook::LoadInitialURL(const GURL& url) {
...
void WebFacebook::SetFastResize(bool fast_resize) {
  holder_->set_fast_resize(fast_resize);
}

void WebFacebook::EnableSizingFromWebContents(const gfx::Size& min_size,
...
void WebFacebook::SetCrashedOverlayView(View* crashed_overlay_view) {
...
////////////////////////////////////////////////////////////////////////////////
// WebFacebook, View overrides:

void WebFacebook::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (crashed_overlay_view_)
    crashed_overlay_view_->SetBoundsRect(gfx::Rect(size()));

  // In most cases, the holder is simply sized to fill this WebFacebook's bounds.
...
void WebFacebook::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details) {
  if (details.is_add)
    AttachWebContentsNativeView();
}

bool WebFacebook::SkipDefaultKeyEventProcessing(const ui::KeyEvent& event) {
...
bool WebFacebook::OnMousePressed(const ui::MouseEvent& event) {
  // A left-click within WebFacebook is a request to focus.  The area within the
...
void WebFacebook::OnFocus() {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->Focus();
}

void WebFacebook::AboutToRequestFocusFromTabTraversal(bool reverse) {
  if (web_contents() && !web_contents()->IsCrashed())
    web_contents()->FocusThroughTabTraversal(reverse);
}

void WebFacebook::AddedToWidget() {
  if (!web_contents())
    return;
...
gfx::NativeViewAccessible WebFacebook::GetNativeViewAccessible() {
  if (web_contents() && !web_contents()->IsCrashed()) {
...
void WebFacebook::OnAXModeAdded(ui::AXMode mode) {
  if (!GetWidget() || !web_contents())
    return;

  // Normally, it is set during AttachWebContentsNativeView when the WebFacebook is
  // created but this may not happen on some platforms as the accessible object
  // may not have been present when this WebFacebook was created. So, update it when
  // AX mode is added.
  UpdateNativeViewHostAccessibleParent(holder(), parent());
}

////////////////////////////////////////////////////////////////////////////////
// WebFacebook, content::WebContentsDelegate implementation:

////////////////////////////////////////////////////////////////////////////////
// WebFacebook, content::WebContentsObserver implementation:

void WebFacebook::RenderFrameCreated(content::RenderFrameHost* render_frame_host) {
...
void WebFacebook::RenderFrameDeleted(content::RenderFrameHost* render_frame_host) {
...
void WebFacebook::RenderFrameHostChanged(content::RenderFrameHost* old_host,
...
void WebFacebook::DidToggleFullscreenModeForTab(bool entered_fullscreen,
...
void WebFacebook::OnWebContentsFocused(
    content::RenderWidgetHost* render_widget_host) {
  RequestFocus();
}

void WebFacebook::AXTreeIDForMainFrameHasChanged() {
  NotifyAccessibilityWebContentsChanged();
}

void WebFacebook::WebContentsDestroyed() {
  SetWebContents(nullptr);
}

void WebFacebook::ResizeDueToAutoResize(content::WebContents* source,
...
void WebFacebook::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kWebView;
  // A webfacebook does not need an accessible name as the document title is
...
void WebFacebook::AttachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebFacebook::AttachWebContentsNativeView");
  // Prevents attachment if the WebFacebook isn't already in a Widget, or it's
  // already attached.
  if (!GetWidget() || !web_contents())
    return;
...
void WebFacebook::DetachWebContentsNativeView() {
  TRACE_EVENT0("views", "WebFacebook::DetachWebContentsNativeView");
  if (web_contents()) {
    holder_->Detach();
  }
}

void WebFacebook::UpdateCrashedOverlayView() {
...
void WebFacebook::NotifyAccessibilityWebContentsChanged() {
...
std::unique_ptr<content::WebContents> WebFacebook::CreateWebContents(
...
void WebFacebook::SetUpNewMainFrame(content::RenderFrameHost* frame_host) {
...
void WebFacebook::LostMainFrame() {
  UpdateCrashedOverlayView();
  NotifyAccessibilityWebContentsChanged();
}

void WebFacebook::MaybeEnableAutoResize(content::RenderFrameHost* frame_host) {
  DCHECK(frame_host->IsRenderFrameLive());
  if (!max_size_.IsEmpty())
    frame_host->GetView()->EnableAutoResize(min_size_, max_size_);
}

BEGIN_METADATA(WebFacebook, View)
END_METADATA
