...
class WebFacebookTestWebContentsObserver : public content::WebContentsObserver {
 public:
  explicit WebFacebookTestWebContentsObserver(content::WebContents* web_contents)
      : web_contents_(web_contents) {
    content::WebContentsObserver::Observe(web_contents);
  }

  WebFacebookTestWebContentsObserver(const WebFacebookTestWebContentsObserver&) =
      delete;
  WebFacebookTestWebContentsObserver& operator=(
      const WebFacebookTestWebContentsObserver&) = delete;

  ~WebFacebookTestWebContentsObserver() override {
    if (web_contents_)
...
// Fakes the fullscreen browser state reported to WebContents and WebFacebook.
class WebFacebookTestWebContentsDelegate : public content::WebContentsDelegate {
 public:
  WebFacebookTestWebContentsDelegate() = default;

  WebFacebookTestWebContentsDelegate(const WebFacebookTestWebContentsDelegate&) =
      delete;
  WebFacebookTestWebContentsDelegate& operator=(
      const WebFacebookTestWebContentsDelegate&) = delete;

  ~WebFacebookTestWebContentsDelegate() override = default;
...
// Provides functionality to test a WebFacebook.
class WebFacebookUnitTest : public views::test::WidgetTest {
 public:
  WebFacebookUnitTest()
      : views::test::WidgetTest(std::unique_ptr<base::test::TaskEnvironment>(
            std::make_unique<content::BrowserTaskEnvironment>())) {}

  WebFacebookUnitTest(const WebFacebookUnitTest&) = delete;
  WebFacebookUnitTest& operator=(const WebFacebookUnitTest&) = delete;

  ~WebFacebookUnitTest() override = default;

  std::unique_ptr<content::WebContents> CreateWebContentsForWebFacebook(
  ...
  void SetUp() override {
    ...
    views::WebFacebook::WebContentsCreator creator = base::BindRepeating(
        &WebFacebookUnitTest::CreateWebContentsForWebFacebook, base::Unretained(this));
    scoped_web_contents_creator_ =
        std::make_unique<views::WebFacebook::ScopedWebContentsCreatorForTesting>(
            creator);
    ...
    // Create a top level widget and add a child, and give it a WebFacebook as a
    // child.
    ...
    auto web_view = std::make_unique<WebFacebook>(browser_context_.get());
 ...
 protected:
  ...
  WebFacebook* web_view() const { return web_view_; }
 ...
 private:
  ...
  std::unique_ptr<views::WebFacebook::ScopedWebContentsCreatorForTesting>
      scoped_web_contents_creator_;

  raw_ptr<Widget> top_level_widget_ = nullptr;
  raw_ptr<WebFacebook> web_view_ = nullptr;
};

// Tests that attaching and detaching a WebContents to a WebFacebook makes the
// WebContents visible and hidden respectively.
TEST_F(WebFacebookUnitTest, TestWebFacebookAttachDetachWebContents) {
  // Case 1: Create a new WebContents and set it in the webfacebook via
  // SetWebContents. This should make the WebContents visible.
  const std::unique_ptr<content::WebContents> web_contents1(
      CreateWebContents());
  WebFacebookTestWebContentsObserver observer1(web_contents1.get());
  ...
  const std::unique_ptr<content::WebContents> web_contents2(
      CreateWebContents());
  WebFacebookTestWebContentsObserver observer2(web_contents2.get());
  ...
  // Case 3: Test that attaching to a hidden webfacebook does not show the web
  // contents.
...
#if defined(USE_AURA)
  // Case 4: Test that making the webfacebook visible when a window has an invisible
  ...
  // window visibility. However, we can still test that moving a visible WebFacebook
  // whose WebContents is not currently showing to a new, visible window will
  // show the WebContents. Simulate the "hide window with visible WebFacebook" step
  ...
  // supported use case. Instead, move the WebFacebook over.
  parent2->SetContentsView(web_view()->parent()->RemoveChildViewT(web_view()));
  EXPECT_EQ(3, observer1.shown_count());
  parent2->Close();
}

// Verifies that there is no crash in WebFacebook destructor
// if WebFacebook is already removed from Widget.
TEST_F(WebFacebookUnitTest, DetachedWebFacebookDestructor) {
  // Init WebFacebook with attached NativeView.
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  std::unique_ptr<WebFacebook> webfacebook(
      new WebFacebook(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  contents_view->AddChildView(webfacebook.get());
  webfacebook->SetWebContents(web_contents.get());

  // Remove WebFacebook from views hierarchy. NativeView should be detached
  // from Widget.
  contents_view->RemoveChildView(webfacebook.get());
  // Destroy WebFacebook. NativeView should be detached secondary.
...
TEST_F(WebFacebookUnitTest, CrashedOverlayView) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());

  std::unique_ptr<WebFacebook> web_view(
      new WebFacebook(web_contents->GetBrowserContext()));
  View* contents_view = top_level_widget()->GetContentsView();
  ...
  // Normally when a renderer crashes, the WebFacebook will learn about it
  // automatically via WebContentsObserver. Since this is a test
  // WebContents, simulate that by calling SetIsCrashed and then
  // explicitly calling RenderFrameDeleted on the WebFacebook to trigger it
...
TEST_F(WebFacebookUnitTest, CrashedOverlayViewOwnedbyClient) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateTestWebContents();
  content::WebContentsTester* tester =
      content::WebContentsTester::For(web_contents.get());
...
// Tests to make sure we can default construct the WebFacebook class and set the
// BrowserContext after construction.
TEST_F(WebFacebookUnitTest, DefaultConstructability) {
  auto browser_context = std::make_unique<content::TestBrowserContext>();
  auto web_view = std::make_unique<WebFacebook>();

  // Test to make sure the WebFacebook returns a nullptr in the absence of an
  ...
  // WebFacebook should be able to create a WebContents object from the previously
  // set |browser_context|.
...
TEST_F(WebFacebookUnitTest, ReparentingUpdatesParentAccessible) {
  const std::unique_ptr<content::WebContents> web_contents =
      CreateWebContents();
  auto web_view = std::make_unique<WebFacebook>(web_contents->GetBrowserContext());
  web_view->SetWebContents(web_contents.get());

  WidgetAutoclosePtr widget_1(CreateTopLevelPlatformWidget());
  View* contents_view_1 = widget_1->GetContentsView();
  WebFacebook* added_web_view = contents_view_1->AddChildView(std::move(web_view));
...
// This tests that we don't crash if WebFacebook doesn't have a Widget or a
// Webcontents. https://crbug.com/1191999
TEST_F(WebFacebookUnitTest, ChangeAXMode) {
  // Case 1: WebFacebook has a Widget and no WebContents.
  SetAXMode(ui::AXMode::kFirstModeFlag);

  // Case 2: WebFacebook has no Widget and a WebContents.
  View* contents_view = top_level_widget()->GetContentsView();
...
// Tests to make sure the WebFacebook clears away the reference to its hosted
// WebContents object when its deleted.
TEST_F(WebFacebookUnitTest, WebFacebookClearsWebContentsOnDestruction) {
