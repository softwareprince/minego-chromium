...
// this file doesnt make a difference so webCoinMarketCap/webCoinMarketCap.h" must be

WebCoinMarketCapSidePanelCoordinator::WebCoinMarketCapSidePanelCoordinator(Browser* browser)
    : BrowserUserData<WebCoinMarketCapSidePanelCoordinator>(*browser) {}
WebCoinMarketCapSidePanelCoordinator::~WebCoinMarketCapSidePanelCoordinator() = default;

void WebCoinMarketCapSidePanelCoordinator::CreateAndRegisterEntry(
    SidePanelRegistry* registry) {
  registry->Register(std::make_unique<SidePanelEntry>(
      SidePanelEntry::Id::kWebCoinMarketCap,
      l10n_util::GetStringUTF16(IDS_SIDEBAR_WEBVIEW_TITLE),
      ui::ImageModel::FromVectorIcon(omnibox::kDinoIcon, ui::kColorIcon),
      base::BindRepeating(&WebCoinMarketCapSidePanelCoordinator::CreateView,
                          base::Unretained(this))));
}
#include "brave/components/sidebar/constants.h"
std::unique_ptr<views::View> WebCoinMarketCapSidePanelCoordinator::CreateView() {
...
//   int random = rand() % 1000000;
  ...
  webcoinmarketcap_ = view->AddChildView(std::move(webcoinmarketcap));

  return view;
}

void WebCoinMarketCapSidePanelCoordinator::PrimaryPageChanged(content::Page& page) {
  location_->SetText(base::UTF8ToUTF16(
      webcoinmarketcap_->GetWebContents()->GetVisibleURL().possibly_invalid_spec()));
}

bool WebCoinMarketCapSidePanelCoordinator::HandleKeyEvent(
    views::Textfield* sender,
    const ui::KeyEvent& key_event) {
  if (key_event.type() == ui::ET_KEY_PRESSED &&
      key_event.key_code() == ui::VKEY_RETURN) {
    webcoinmarketcap_->GetWebContents()->GetController().LoadURL(
...
WEB_CONTENTS_USER_DATA_KEY_IMPL(WebCoinMarketCapSidePanelCoordinator);
