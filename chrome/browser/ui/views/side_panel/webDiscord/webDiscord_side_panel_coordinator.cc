...
// this file doesnt make a difference so webDiscord/webDiscord.h" must be

WebDiscordSidePanelCoordinator::WebDiscordSidePanelCoordinator(Browser* browser)
    : BrowserUserData<WebDiscordSidePanelCoordinator>(*browser) {}
WebDiscordSidePanelCoordinator::~WebDiscordSidePanelCoordinator() = default;

void WebDiscordSidePanelCoordinator::CreateAndRegisterEntry(
    SidePanelRegistry* registry) {
  registry->Register(std::make_unique<SidePanelEntry>(
      SidePanelEntry::Id::kWebDiscord,
      l10n_util::GetStringUTF16(IDS_SIDEBAR_WEBVIEW_TITLE),
      ui::ImageModel::FromVectorIcon(omnibox::kDinoIcon, ui::kColorIcon),
      base::BindRepeating(&WebDiscordSidePanelCoordinator::CreateView,
                          base::Unretained(this))));
}
#include "brave/components/sidebar/constants.h"
std::unique_ptr<views::View> WebDiscordSidePanelCoordinator::CreateView() {
...
//   int random = rand() % 1000000;
  ...
  webdiscord_ = view->AddChildView(std::move(webdiscord));

  return view;
}

void WebDiscordSidePanelCoordinator::PrimaryPageChanged(content::Page& page) {
  location_->SetText(base::UTF8ToUTF16(
      webdiscord_->GetWebContents()->GetVisibleURL().possibly_invalid_spec()));
}

bool WebDiscordSidePanelCoordinator::HandleKeyEvent(
    views::Textfield* sender,
    const ui::KeyEvent& key_event) {
  if (key_event.type() == ui::ET_KEY_PRESSED &&
      key_event.key_code() == ui::VKEY_RETURN) {
    webdiscord_->GetWebContents()->GetController().LoadURL(
...
WEB_CONTENTS_USER_DATA_KEY_IMPL(WebDiscordSidePanelCoordinator);
