...
// this file doesnt make a difference so webBinance/webBinance.h" must be

WebBinanceSidePanelCoordinator::WebBinanceSidePanelCoordinator(Browser* browser)
    : BrowserUserData<WebBinanceSidePanelCoordinator>(*browser) {}
WebBinanceSidePanelCoordinator::~WebBinanceSidePanelCoordinator() = default;

void WebBinanceSidePanelCoordinator::CreateAndRegisterEntry(
    SidePanelRegistry* registry) {
  registry->Register(std::make_unique<SidePanelEntry>(
      SidePanelEntry::Id::kWebBinance,
      l10n_util::GetStringUTF16(IDS_SIDEBAR_WEBVIEW_TITLE),
      ui::ImageModel::FromVectorIcon(omnibox::kDinoIcon, ui::kColorIcon),
      base::BindRepeating(&WebBinanceSidePanelCoordinator::CreateView,
                          base::Unretained(this))));
}
#include "brave/components/sidebar/constants.h"
std::unique_ptr<views::View> WebBinanceSidePanelCoordinator::CreateView() {
...
//   int random = rand() % 1000000;
  ...
  webbinance_ = view->AddChildView(std::move(webbinance));

  return view;
}

void WebBinanceSidePanelCoordinator::PrimaryPageChanged(content::Page& page) {
  location_->SetText(base::UTF8ToUTF16(
      webbinance_->GetWebContents()->GetVisibleURL().possibly_invalid_spec()));
}

bool WebBinanceSidePanelCoordinator::HandleKeyEvent(
    views::Textfield* sender,
    const ui::KeyEvent& key_event) {
  if (key_event.type() == ui::ET_KEY_PRESSED &&
      key_event.key_code() == ui::VKEY_RETURN) {
    webbinance_->GetWebContents()->GetController().LoadURL(
...
WEB_CONTENTS_USER_DATA_KEY_IMPL(WebBinanceSidePanelCoordinator);
